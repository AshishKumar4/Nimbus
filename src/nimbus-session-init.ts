/**
 * nimbus-session-init.ts — initSession boot + 19 shell-cmd registrations.
 *
 * Extracted from src/nimbus-session.ts per
 * audit/sections/SESSION-REFACTOR-PLAN.md §B.3.8 + S6.
 *
 * This is the largest single module of the refactor (~1875 LOC).
 * The function takes `self: SessionInternal` and is mechanically the
 * original `private initSession` method body with `this.` → `self.`
 * and the inner `const self = this;` removed (param shadows).
 *
...
import { SqliteVFSProvider } from './sqlite-vfs.js';
...
} from './nimbus-session-helpers.js';
...
import { stripAnsi, type LogChunk } from './process-logs.js';
import {
...
import { VITE_CONFIG_KEY } from './nimbus-session-keys.js';
