/**
 * nimbus-session-helpers.ts — pure helpers extracted from nimbus-session.ts.
 *
 * All functions in this module are pure (no class state, no
 * `cloudflare:workers` import), so they can be unit-tested with bun test
 * without needing a workerd harness.
 *
 * Originally lines 96-523 of src/nimbus-session.ts. Per
