 ...
 *
 * X.5-L: thin wrapper around resolvePkgSubpathEx that drops the stub
 * info. Kept for back-compat with relative-path callers that don't
 * exercise the legacy nested-pkg branch.
 */
function resolvePkgSubpath(vfs: SqliteVFS, pkgDir: string, subpath: string): string | null {
  const r = resolvePkgSubpathEx(vfs, pkgDir, subpath);
...
function resolveNodeModule(vfs: SqliteVFS, name: string, fromDir: string): string | null {
  const r = resolveNodeModuleEx(vfs, name, fromDir);
  return r ? r.resolved : null;
}
...
function resolveRequire(vfs: SqliteVFS, id: string, fromDir: string): string | null {
  const r = resolveRequireEx(vfs, id, fromDir);
  return r ? r.resolved : null;
}
