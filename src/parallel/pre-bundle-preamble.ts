/**
 * pre-bundle-preamble.ts — preamble injected into NimbusFacetPool isolates
 ...
 *     NimbusFacetPool's `wasmModules` option (LOADER `modules` map
...
// NOTE: ESBUILD_WASM_BASE64 deliberately NOT imported here. Earlier
...
//   2. Fetch bytes at request time via SUPERVISOR.getEsbuildWasm RPC
//      and call WebAssembly.compile() in the facet — workerd's
//      deployed config disallows wasm code generation at request
//      time ("Wasm code generation disallowed by embedder").
...
//
// Working approach (current): wasm bytes ride INSIDE the LOADER
// `modules` map as `{ wasm: ArrayBuffer }`. workerd compiles during
// the worker's module-load phase (where wasm code generation is
// permitted) and exposes the Module via the standard ESM import the
// pool prepends to worker.js. See NimbusFacetPool.wasmModules option
// in src/parallel/facet-pool.ts and the npm-installer dispatch site.
 ...
 * pre-bundle facet isolate. Must be passed via NimbusFacetPool's
 * `preamble` option.
 */
