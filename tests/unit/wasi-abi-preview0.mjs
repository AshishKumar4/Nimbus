...
import { WASI_ABI_NAMESPACE } from '../../packages/core/src/runtime/wasi-instance.ts';
import { loadWasiPreamble, makeGuest, makeSession } from './lib/wasi-authority.mjs';

const P = await loadWasiPreamble();
const sessions = [];

/** A fresh guest over a fresh session holding one `fileSize`-byte file at the root. */
function freshLayer({ abi, fileSize }) {
  const session = makeSession({ files: { 'probe.bin': new Uint8Array(fileSize) } });
  sessions.push(session);
  const guest = makeGuest(P, session, {}, { abi });
  const { errno, fd } = guest.open('probe.bin');
  assert.equal(errno, 0, 'path_open should succeed');
  return { wasiImport: guest.wasi, dv: guest.dv(), u8: guest.u8(), fd };
}
...
{
  const { wasiImport, dv, fd } = freshLayer({ abi: 'preview0', fileSize: 100 });
  const out = 4096;
...
{
  const { wasiImport, dv, fd } = freshLayer({ abi: 'preview1', fileSize: 100 });
  const out = 4096;
...
{
  const { wasiImport, dv, u8, fd } = freshLayer({ abi: 'preview0', fileSize: 4242 });
  const statPtr = 8192;
...
{
  const { wasiImport, dv, u8, fd } = freshLayer({ abi: 'preview1', fileSize: 4242 });
  const statPtr = 8192;
  u8.fill(0xab, statPtr, statPtr + 72);
  assert.equal(wasiImport.fd_filestat_get(fd, statPtr), 0);
  assert.equal(dv.getUint8(statPtr + 16), 4, 'filetype must be REGULAR_FILE');
