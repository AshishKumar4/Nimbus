...
// code. It runs against the REAL preamble from wasi/preamble.ts over a real
// session filesystem (lib/wasi-authority.mjs) — the same authority, the same
// syscalls — so a regression in either half shows up here.
//
// The guest runs the way a loader facet runs it: every entry goes through
// WebAssembly.promising and the imports are Suspending, so a syscall that
// parks (poll_oneoff, a socket) suspends the interpreter and resumes it. The
// authority is in this isolate, so file syscalls answer without parking; the
// RPC-parked file path is cpython-live-authority.mjs.
//
// A red here can still be about the harness rather than the runtime: CPython
// emits "Failed to import encodings module" for every variety of "I cannot
// find my stdlib", so read the errno trail before blaming the interpreter.
//
...
const session = makeSession({
  ...
  files: {
    ...
    'work/pip.whl': readFileSync(PIP_WHEEL),
  },
});
...
const run = (src) => withCString(src, (ptr) => enter(nimbus_py_run)(ptr));
...
// writes had to be diffed back out. These go straight through the authority.
assert.equal(await run(`
import os
...
assert.equal(session.user.readFileString('work/note.txt'), 'written by python',
  'and the bytes are in the session, readable by the shell');
...
assert.equal(await run('steps = []\ndef work():\n'
  + '    for i in range(3):\n        steps.append(i)\n        yield i\ng = work()'), 0);
for (let turn = 0; turn < 3; turn++) {
  assert.equal(await run('next(g)'), 0, `generator resume ${turn} failed: ${stderr.join('')}`);
}
assert.equal(await run('assert steps == [0, 1, 2], steps\nprint("resumed", len(steps), "times")'), 0,
...
assert.equal(await run('raise SystemExit(0)'), 0, 'SystemExit(0) is success');
assert.equal(await run('raise SystemExit(3)'), 3, 'SystemExit carries its status');
assert.equal(await run('raise SystemExit()'), 0, 'a bare SystemExit is success');
assert.equal(await run('import sys; sys.exit(7)'), 7, 'sys.exit is the same path');
stderr.length = 0;
assert.equal(await run('raise ValueError("deliberate")'), 1, 'any other exception is failure');
...
assert.equal(await run('print("still here", steps)'), 0);
...
assert.ok(session.user.exists('work/site/nimbusprobe/__init__.py'), 'pip wrote the package into the session');
