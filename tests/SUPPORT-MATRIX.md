...
| WASI preview1 / wasm runner | ✅ | `tests/behavioral/wasi/*`, `tests/behavioral/wasi-paths/*`, `tests/behavioral/wasm-runner/*` | All 46 preview1 functions implemented; outbound TCP, inbound accept and poll paths are probed. |
| wasi-threads / `-pthread` | ✅ | `tests/unit/wasi-threads-scheduler.mjs`, `tests/unit/wasi-threads-load-contract.mjs`, `tests/behavioral/wasm/pthread-parity.mjs`, `tests/behavioral/clang/pthread-refused.mjs` | Mutex, condvar (signal and broadcast), join, TLS, barriers, semaphores, rwlocks, once-init, detach, `cond_timedwait` and `trylock` are each probed against the exact line the same C source prints under a real POSIX pthread implementation. Cooperative only — no parallelism, and no preemption of a thread that never blocks. Builds missing the shared memory or the futex shim are rejected at load, as is a shim linked against wasi-libc older than wasi-sdk 27, which drops it silently. Threaded binaries must be built off-platform: the in-session clang has no threads sysroot and refuses `-pthread`. |
