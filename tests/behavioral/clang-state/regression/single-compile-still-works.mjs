#!/usr/bin/env bun
// clang-state/regression/single-compile-still-works — canonical
// single compile + run baseline preserved.

import { mintSession, Terminal, makeAsserter, heredocCommand, stripAnsi } from '../../_driver.mjs';

if (!process.env.BASE) { console.error('FATAL: BASE env required'); process.exit(2); }
const a = makeAsserter('clang-state/regression/single-compile-still-works');
