...
A passing probe asserting on the wrong thing is **strictly worse than no
...
broken in the same way. This is exactly the false-positive we hit on
...
ever been seen passing and has never failed against a real bug is
suspect by default.**

## False-passing incident — 2026-05-10

The framework-validation wave's `frameworks/markflow-clickthrough.mjs`
and `frameworks/cjs-esm-interop.mjs` were both 100% passing against prod
