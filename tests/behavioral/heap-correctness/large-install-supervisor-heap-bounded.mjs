...
// What we assert:
//
//   passing — peak heap.bytes during install stays below a documented
...
//   failing — peak vfsInFlightBytes grows unbounded with package count,
...
const verdict = (() => {
  ...
  // counter. So this probe is failing until both N2 + N3 ship together.
  if (peakInFlight === 0) return { state: 'failing', reason: 'peakInFlightWriteBytes always 0 — counter is hardcoded (N3) so N2 invisible' };
  ...
  if (peakInFlight > CEILING) {
    return { state: 'failing', reason: `peakInFlightWriteBytes=${fmtBytes(peakInFlight)} > ${fmtBytes(CEILING)}` };
  }
  return { state: 'passing', reason: `peakInFlightWriteBytes=${fmtBytes(peakInFlight)} ≤ ${fmtBytes(CEILING)}` };
})();
