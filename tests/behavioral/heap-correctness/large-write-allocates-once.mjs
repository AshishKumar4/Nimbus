...
const verdict = (() => {
  ...
  if (peakInFlight === 0) {
    return { state: 'failing', reason: `peakInFlightBytes=0 across ${samples.length} samples during a ${(elapsed/1000).toFixed(1)}s install — counter is hardcoded` };
  }
  if (peakInFlight > CEILING) {
    return { state: 'failing', reason: `peakInFlightBytes=${fmtBytes(peakInFlight)} > ceiling ${fmtBytes(CEILING)}` };
  }
  return { state: 'passing', reason: `peakInFlight=${fmtBytes(peakInFlight)} (pending=${fmtBytes(peakPendingBytes)}, spool=${fmtBytes(peakSpoolBytes)}) ≤ ${fmtBytes(CEILING)}` };
})();
