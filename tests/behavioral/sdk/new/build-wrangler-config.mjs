#!/usr/bin/env bun
// sdk/new/build-wrangler-config — @nimbus-sh/config produces a
// valid wrangler.jsonc with the expected bindings shape.

import { makeAsserter } from '../../_driver.mjs';
const a = makeAsserter('sdk/new/build-wrangler-config');

const { buildNimbusWranglerConfig, defineNimbusConfig, NIMBUS_REQUIRED_ALIASES } =
...
// 1. Minimum-input config has all the required bindings.
{
  ...
  a.check('assets.run_worker_first covers API, sessions, and /new',
    c.assets.run_worker_first?.includes('/api/*')
...
}
{
  const c = buildNimbusWranglerConfig({
    name: 'byo',
    runtimeCache: { mode: 'byoa', bucket: 'custom-runtime-cache' },
  });
  const rt = c.r2_buckets.find((b) => b.binding === 'NIMBUS_RUNTIME_CACHE');
  a.check('runtime cache object bucket override', rt.bucket_name === 'custom-runtime-cache');
}
...
// 8. defineNimbusConfig preserves sandbox profiles for SDK consumption.
{
  const cfg = defineNimbusConfig({
    ...
    sandboxes: {
      proteus: {
        ...
        runtimes: {
          ...
          onDemand: true,
          allow: ['node', 'python', 'clang'],
        },
        tools: { namespace: 'sandbox', kind: 'sandbox' },
      },
    },
  });
  a.check('defineNimbusConfig preserves endpoint', cfg.endpoint === 'https://nimbus.example.com');
  a.check('defineNimbusConfig preserves preinstall',
    cfg.sandboxes.proteus.runtimes.preinstall.join(',') === 'python,clang');
  a.check('defineNimbusConfig preserves tools namespace',
