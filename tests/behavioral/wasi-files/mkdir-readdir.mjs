...
console.log(`[wasi-files/mkdir-readdir] sid=${sid} BASE=${BASE}`);
...
console.log(JSON.stringify({ probe: 'wasi-files/mkdir-readdir', sid, base: BASE, tail, ok }, null, 2));
...
console.log(`[wasi-files/mkdir-readdir] ${verdict} — ${pass}/${checks.length}`);
