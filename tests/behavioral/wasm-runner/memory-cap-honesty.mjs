...
// checks the two things that matter: the program itself reports the failure,
// and the session is still alive afterwards. The second is the whole point —
// uncapped, this program produces no output to assert on, because it takes
// the session with it.

import {
  mintSession, Terminal, makeAsserter, stripAnsi, heredocCommand, deleteSession,
} from '../_driver.mjs';
...
//     outright — an earlier draft of this probe reported "never refused" from
//     a binary whose memory never grew past its initial 17 pages.
//   - Counters are `int` chunk counts, not byte totals. `long` is 32 bits on
//     wasm32, so a byte total wraps to exactly 0 at 4 GiB.
//
// It also reports `memory_size` directly, so a run that never grew can never
// again be mistaken for a run that grew and was refused.
const HOG_C = `#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
try {
  ...
  await t.run(heredocCommand('hog.c', HOG_C), 15_000);
...

  {
    ...
    // DEFAULT_WASM_PROCESS_LIMIT_BYTES is 128 MiB = 2048 pages. Stopping just
    // below that is the cap doing its job; sailing past it means the cap never
    // reached the module, and stopping near zero would mean it landed below
    // what the module needs to be useful.
    a.check('it stopped below the 2048-page (128 MiB) declared maximum',
      pages !== null && pages <= 2048, `grew to ${pages} pages`);
    a.check('after being given a useful amount of memory — at least 32 MiB',
      refusedAt !== null && refusedAt >= 32, `refused at ${refusedAt} MiB`);
  }

  // The reason this change exists. An uncapped grow kills the isolate hosting
  // the process, and the session is what the user notices losing.
  {
  ...
  if (!cleanup.ok) console.warn(`cleanup failed for ${sid}: ${cleanup.status} ${cleanup.body}`);
}
