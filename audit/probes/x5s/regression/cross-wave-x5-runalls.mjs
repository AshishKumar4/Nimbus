#!/usr/bin/env bun
// X.5-S regression: every prior X.5 wave run-all must remain green
// (with previously-known pre-existing fails preserved but not new ones).
//
// Mirrors audit/probes/x5m3/regression/cross-wave-x5-runalls.mjs,
// extended to include x5m3 (now part of HEAD).
//
...
// Per X5Z3-retro §6 / X5M3 cross-wave probe: x5z5-build has a pre-existing
// `tailwindcss-vite e2e` FAIL gated on a different fix class (lightningcss
// native binding — wasm-swap-registry territory). Mark expected-fail at
// the run-all level and gate on its presence rather than its overall exit.
const suites = [
  ...
  // X5Z5-build-retro §1 + X5Z3-retro §6. Not an X.5-S regression.
  ...
  { name: 'x5m3',       path: 'audit/probes/x5m3/run-all.mjs',       expectedExit: 0 },
];
...
// invoke separately from the x5s run-all driver).
