#!/usr/bin/env bun
// Classify each packages-local/<pkg>.out.txt artifact for verify-700420f.
// Forked verbatim from VERIFY-90993B3's classify-packages-local.mjs so
// classification rules are byte-identical between waves.
...
function classify(text) {
  ...
  else if (installStatus === 'rejected-loud') overall = '⛔';
  else if (installStatus === 'noop-skip') overall = '❌';
...
fs.writeFileSync(path.join(DIR, '_TABLE.md'), [
  `# verify-700420f local package compat — generated ${new Date().toISOString()}`,
  '',
