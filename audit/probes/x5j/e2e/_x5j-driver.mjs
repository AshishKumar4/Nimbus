// X5J e2e driver: install + require a single package via wrangler dev.
// Reuses the post-phase5-verification local probe pattern (BASE must
// be set to http://127.0.0.1:8787; per AGENTS.md, port 8787 +
// --ip 0.0.0.0).
//
// Usage:
//   BASE=http://127.0.0.1:8787 bun audit/probes/x5j/e2e/<probe>.mjs
...
export async function runOnePkg({ name, pkg, smoke, expectations }) {
  if (!process.env.BASE) {
    console.error(`FATAL: must set BASE (e.g. http://127.0.0.1:8787) for x5j e2e probes.`);
    process.exit(2);
  }
  ...
  console.log(`[X5J E2E] ${name} starting…`);
  const id = `x5j_${Date.now().toString(36)}`;
  ...
  let verdict = 'unknown';
  if (expectations.expectReject) {
    // For loud-reject regression probes: install MUST be rejected.
    if (/npm install rejected:/.test(out)) verdict = '⛔ loud-reject (expected)';
    else verdict = '? UNEXPECTED — reject not seen';
  } else {
  ...
  console.log(`[X5J E2E] ${name} → ${verdict}`);

  let ok = false;
  if (expectations.expectReject) ok = verdict.startsWith('⛔ loud-reject (expected)');
  else                            ok = verdict.startsWith('✅');
