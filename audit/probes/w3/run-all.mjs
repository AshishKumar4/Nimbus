// W3 — run all probes (functional + regression + e2e)
//
// Usage:
//   bun audit/probes/w3/run-all.mjs                          # default: prod
//   BASE=http://localhost:8787 bun audit/probes/w3/run-all.mjs   # local
//   bun audit/probes/w3/run-all.mjs --only=crypto-real-sha256 # one
//   bun audit/probes/w3/run-all.mjs --skip-e2e                # skip slow e2e
//
// Reports total/passed/failed/skipped, exits non-zero if any failure.
// Output captured to audit/probes/w3/_results/<probe>.out.txt and
// _SUMMARY.json.
...
async function main() {
  ...
  console.log(`W3 run-all — BASE=${base} concurrency=${concurrency}`);
  ...
  if (probes.length === 0) {
    console.log('No probes matched. Exit 0.');
    process.exit(0);
  }
  ...
  console.log('');
  console.log(`========================================`);
  console.log(`W3 run-all summary:`);
