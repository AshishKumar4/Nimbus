#!/usr/bin/env bun
// verify-90993b3 package harness — same TARGETS as
// audit/probes/post-phase5-verification/run-packages-local.mjs but writes
// artifacts under audit/probes/verify-90993b3/packages-local/.
//
// Single-target mode preferred to avoid wrangler workerd OOM mid-run:
//   --only=<name>   run a single target (used by batch wrapper)
//   --skip-existing skip probes whose artifact is non-empty + not POST /new FAILED
