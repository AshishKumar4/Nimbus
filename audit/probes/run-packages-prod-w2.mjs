// W2 prod-verification runner.
//
// Re-runs the same TARGETS list as run-packages.mjs but writes outputs to
// audit/probes/packages-prod-w2/ so the pre-W2 baseline in
// audit/probes/packages/ is preserved unchanged.
//
// Usage:
//   bun audit/probes/run-packages-prod-w2.mjs                # all 33
//   bun audit/probes/run-packages-prod-w2.mjs --only=react   # one
//   bun audit/probes/run-packages-prod-w2.mjs --skip-existing
//
// Output: audit/probes/packages-prod-w2/<name>.out.txt + .probe.js
//         audit/probes/packages-prod-w2/_SUMMARY.json
...
const OUT_DIR = path.join(HERE, 'packages-prod-w2');
fs.mkdirSync(OUT_DIR, { recursive: true });

// Same TARGETS as run-packages.mjs — kept literal so the post-W2 run is
// trivially comparable to the pre-W2 baseline.
const TARGETS = [
