...
// [CWB-1 hotfix, 2026-05-05] `wrangler deploy --env production` so the
// env.production overlay (which carries `replica_routing`) is applied
// to the deployed Worker. The top-level wrangler.jsonc no longer carries
// `replica_routing`, so a bare `wrangler deploy` (no --env) would lose
// the W12 GA flag. See audit/sections/POST-PHASE5-CROSS-WAVE-AUDIT §CWB-1
// and the env.production block in wrangler.jsonc for the full root cause.
let versionId = null;
let deployOk = false;
if (!SKIP_DEPLOY) {
  console.log('\n# Step 3: wrangler deploy --env production');
  const r = wrangler(['deploy', '--env', 'production'], {
    capture: true,
  ...
  if (!deployOk) {
    console.error('\n[WARN] wrangler deploy --env production failed.');
    console.error('  - If the failure mentions `replica_routing`, the account is not on the GA');
    console.error('    allowlist. Edit wrangler.jsonc env.production.compatibility_flags to remove');
    console.error('    `replica_routing` and rerun with --skip-baseline.');
    console.error('  - If the failure mentions a missing binding, env.production may have drifted');
    console.error('    from top-level. The non-inheritable keys (vars / durable_objects / r2_buckets)');
    console.error('    must be redeclared in env.production. See wrangler.jsonc inline comments.');
