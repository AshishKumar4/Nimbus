...
Each launch primitive declares its process class, and one policy point reads
it. The serve half binds a port, so it declares light and runs on the
coordinator. The attach half serves no inbound HTTP — frames leave over the
terminal RPC and keystrokes arrive over the stdin pump — so it declares heavy
and runs on a [peer DO](/docs/internals/fabric/) with a memory budget of its
own. It is the only heavy tenant today. The serve half is stable; the attach
half is [still being hardened](/docs/fine-print/capabilities/).
