...
Both halves run the way every resident process does, as a [facet of the
session](/docs/internals/fabric/) with a memory envelope of its own. The
serve half binds a port and answers over it. The attach half sends frames
over the terminal RPC and takes keystrokes over the stdin pump. The serve
half is stable. The attach half is [still being
