...
coordinator. The attach half sends frames over the terminal RPC and receives
keystrokes over the stdin pump, so it declares heavy and runs on a [peer
DO](/docs/internals/fabric/) with a memory budget of its own — the only heavy
tenant today. The serve half is stable; the attach half is [still being
