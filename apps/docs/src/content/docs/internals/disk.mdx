...
source of truth, and it shrinks itself under memory pressure.
...
supervisor as length-prefixed, checksummed frames, and a weighted credit
...
through the stream. Waiters are served in FIFO order,
so a 52,500-file `npm install` and an 84,000-file checkout can run in
parallel inside 128 MiB isolates without either starving the other.
...
bytes per transaction. A single SQLite transaction then swaps the
...
4,000 files with `node_modules` excluded, plus a local write cache.
...
The coherence rule is that every read sees the latest committed revision
or its own unflushed write. Conflicts fail loudly rather than overwriting
data silently.
