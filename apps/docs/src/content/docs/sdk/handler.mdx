...
`createNimbusHandler` is the Worker entry point. Its options set the auth
mode, the remote API, your own routes, and the session hooks.
...
`sdk.remote` mounts `/api/nimbus/v1/sandboxes/<id>/rpc`. It is off by
default. Nimbus ships no token-mint endpoint. Use `routes` to add
one behind your own user auth.
...
the required bundler aliases. Use `NIMBUS_REQUIRED_ALIASES` when keeping
the config by hand.
...
`runtime sync`. `mintToken`, `syncRuntimes`, `setupCloudflare`, and
`scaffold` are also callable from code.
