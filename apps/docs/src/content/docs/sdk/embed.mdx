...
The deploy takes three commands plus one secret. `setup cloudflare`
...
Use the SDK through your own DO binding, or connect to an authenticated
deployment.
...
The sandbox profile is a real policy boundary. Nimbus enforces
...
`@nimbus-sh/react` puts the session shell in an iframe. This keeps your
...
`useNimbusSession` gives you the same flow without the component. It
returns `{ sessionId, ready, error, attachUrl }` from the shell's
`nimbus:ready` / `nimbus:error` postMessages, so you can build the iframe.

Nimbus is a Worker/DO/WASM sandbox, not a Linux VM. I list the boundaries
in [the fine print](/docs/fine-print/capabilities/). The hosted demo uses
