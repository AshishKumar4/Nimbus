...
| One-shot `node` bundle build runs in one DO turn | The exec path (`node -e`, `node file.js` for a non-server, an `npm run` script's bin) assembles its filesystem bundle with no turn pacer and races a wall-clock deadline, while the resident path pages the same build across Durable Object turns with `TurnBudget` and no deadline. Measured 2026-09-14 on a 752-package tree: 14.6 s cold, 1.6 s cached, under the deadline; the deadline now scales 20 ms per installed package from a 20 s floor to a 60 s cap (`bundleBuildDeadlineMs`), and the speculative main-entry oversample is bounded to one runtime-dependency hop, so the tree-proportional cost is mostly gone. | The exec path should page its bundle build across turns the way a resident launch does, so a large tree costs turns rather than a single held thread and the wall-clock guard becomes unnecessary. Not done: exec is request-scoped today and moving it onto alarm-driven turns changes one-shot command lifecycle. |
