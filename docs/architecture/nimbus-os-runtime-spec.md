...
| Package self-reference through `exports` | Inside a package's own directory, `require('<its-name>')` / `import('<its-name>')` does not resolve through the package's own `exports` map (Node does: `import('got')` from got's repo resolves to `./dist/source/index.js`). Nimbus reports `No such module "<name>"`; Node reports `ERR_MODULE_NOT_FOUND` for the unbuilt `dist/` in the same scenario, so the outcome matched by accident on the measured case. | The require resolver should try the enclosing package's `exports` for its own name before walking `node_modules`, as Node's self-reference rule specifies. Separate item; not fixed here. |
