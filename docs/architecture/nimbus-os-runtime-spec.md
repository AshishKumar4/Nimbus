...
| One-shot `node` bundle build runs in one DO turn | Closed on this branch, pending a production probe. The exec path (`node -e`, `node file.js` for a non-server, an `npm run` script's bin) and the resident launch path build through one paced builder (`FacetManager._buildProcessBundle`): the walk, enrichment passes, ESM transform and module-map serialization yield the turn every `TurnBudget` chunk and resume on the session's alarm, exactly as a resident launch does; the wall-clock deadline (`bundleBuildDeadlineMs`) and its per-package scaling are gone. Measured 2026-09-14 on a throwaway probe worker against the same 752-package tree (got), train tip vs this branch: `node -e` cold 5.5 s → 5.7 s taking 6 alarm turns (tail: six alarm invocations carrying the build's CPU, the WebSocket message that received the command spent 43 ms), warm 0.5-0.6 s → 0.6-0.8 s on a cache hit with 0 turns; no deadline failure either side. A process killed while its build is suspended stops the build at the next resume, and a build that fails exits its process entry once instead of leaving it running. `npx del-cli --help` still fails in that tree with `Cannot use 'import.meta' outside a module` on both builds — an ESM bin transform gap tracked separately, not the deadline. | The exec path pages its bundle build across turns the way a resident launch does. The probe `tests/behavioral/runtime-primitives/node-e-large-tree.mjs` passed twice on the throwaway (cold 5.4 s / 4.3 s, warm 0.6 s). Remaining: a production run of that probe. A 1085-package tree (react-scripts) measured `node -e "1"` cold 0.85 s in 1 turn, warm 0.4 s; a 1500+-package tree was not reached. |
| Package self-reference through `exports` | Implemented in the shared require/import resolver (runtime `__resolvePackageSelf` in `packages/worker/src/runtime/node-shims.ts`, prefetch `resolvePackageSelf` in `packages/core/src/runtime/require-resolver.ts`, rule in `packages/core/src/_shared/exports-resolver.ts`): inside a package's own directory, `require('<its-name>')` and `require('<its-name>/sub')` resolve through the nearest enclosing package.json's own `exports` map — only when that package.json has `exports` and its `name` matches, before the `node_modules` walk and after relative and `#imports` specifiers; a nearer package of a different name is the scope and is never walked past, the scope walk stops at a `node_modules` directory, and once the enclosing package claims the name its map is the whole answer (a subpath it does not expose is not found, never a `node_modules` copy's). `tests/unit/package-self-reference.mjs` is differential against real Node for both resolvers; `tests/behavioral/runtime-primitives/package-self-reference.mjs` probes it live. | Remaining, package-wide and not specific to self-reference: dynamic `import()` is lowered onto the require chain, so a map with both `import` and `require` conditions answers `import()` with the `require` target; `import()` in a CommonJS-shaped entry (`node -e`, `.js` without `"type": "module"`) goes to workerd's module registry, not the resolver; conditions are matched by the resolver's condition list rather than by key order in the map. |
...
|---|---|---|---|
| Process and PTY | `SessionProcessSupervisor` over `ProcessTable`, `ProcessLogStore`, `ProcessInputStore`, `process-logs-api.ts`, `FacetProcessManager`, `node-shims.ts` child process RPC | One supervisor facade owns PID lifecycle, logs, input packets, process terminals, child-process broker, and attached npm-bin processes; all callers route through it | Add process groups, controlling TTY, raw/cooked mode, durable process metadata, and attach/detach replay on the facade's stage-2 seams |
| Filesystem | `SqliteVFS`, `RuntimeFsBridge`, `SqliteRuntimeFsBridge`, VFS events, `fs-watch.ts` | Durable SQLite-backed VFS with stateless `readRange`/`writeRange`/`truncate`, per-path revisions, and the runtime bridge wired into Node async fs | Move WASI, Python, and Ruby long-running IO onto the live bridge, and add hibernation-safe runtime handles and live runtime cache invalidation |
| Networking and preview | `PortRegistry`, the typed `virtual-socket-kernel` module, Python/Ruby socket shims, session `/port` and `/preview` routes | Request/Response preview gateway plus in-facet loopback sockets for Python/Ruby, built from a typed kernel module with bounded queues and abort handling | Move Node and WASI onto the kernel, finish streaming/backpressure (stage 2), and add hibernation-aware port metadata |
...
shareable. Durability is no longer something an embedder buys with a port
reservation up front — it is a property every resident has from the moment
it is spawned, and a reservation is what *exposing* it creates.
...
- Node, Ruby socket and CPython socket residents have **derived** identity at spawn:
  ...
  server.js` from the same directory is the same application across a
  restart and across a supervisor reset. The owner is stamped on the
  resident's journal row when it is written (`facets/resident-identity.ts`,
  `_runResidentLaunch`).
...
- `_registerResidentPort` stamps the registering pid's journal row with
  `{ port }` whether or not anything reserved the port. Scoped URLs
  (`<port>--<sid>`, `/s/<sid>/port/<n>/`) need no capability, so this alone
  makes every server's scoped preview survive a reset: `ensureDurableAppOnPort`
...
`expose(target, { visibility?, name? })` — target a port, a pid, a name or an
owner (`session/programmatic.ts`, one implementation behind
...
- answers `{ owner, name, port, pid, capability, visibility, url }`.
...
re-derived from the reservation on every launch and never journalled. A
server that ignores `$PORT` still works, because identity is not the port:
it is registered on whatever it bound, but the row records the mismatch, the
user is told (`listened on 3000 but its reservation owns 5173`), and
`apps.list` reports it `failed` with `diagnostic: "listened on 3000, owns
5173"`. A resident spawned before any reservation exists keeps its ephemeral
`proc-slot` facet — the store cannot be re-rooted under a running facet.
...
resolves the name to a port from the session's own reservation records and
...
`startProcess(cmd, { restart: 'never' | 'on-failure' })` (SDK and remote API),
`nimbus start --restart <policy> <command…>` (shell): the policy rides the
...
### The reset probes need

`POST /api/_diag/abort`, gated by `NIMBUS_DEBUG` and the `session:admin`
scope, calls `ctx.abort()`, which ends the DO isolate the way the platform's
own reset does while every synced storage row survives.

### The probes

- `universal-durability` (unit, facet-host harness) — unconditional stamp and
  re-drive with no reservation; derived-owner stability across re-spawn and
  env change; the ephemeral duplicate; lazy expose reserving and binding;
  the capability not inherited by an unrelated server; rotate; remove
  purging process, reservation, rows, slot and directory; the name host
  parse matrix and router/session resolution; `$PORT`/`$NIMBUS_APP`
  injection and the mismatch diagnostic; restart on-failure within budget,
  never/clean/kill releasing; `apps.list` shapes.
- `durable-port-recovery`, `durable-public-port`,
  `resident-worker-reservation` (unit) — the explicit-owner contract, unchanged.
- `durable/universal-scoped-survival` (behavioral) — an unnamed
  `node server.js`, no expose ever called: scoped path URL → abort → the
  same URL answers again from a re-driven process.
