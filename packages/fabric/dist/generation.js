 ...
 */
export function assumeGeneration(ctx, value) {
    const state = stateOf(ctx);
    state.value = value;
    state.adopted = false;
}
 ...
 */
export function onColdStart(ctx, task) {
 ...
 * next call.
 */
export function runColdStart(ctx) {
    const state = stateOf(ctx);
    if (state.coldStart.length === 0)
        return state.coldStartChain;
