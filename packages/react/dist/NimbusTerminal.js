 ...
 */
export const NimbusTerminal = forwardRef(function NimbusTerminal(props, ref) {
    const { endpoint, token, tenant, sub, sessionId, onReady, onError, style, className, sandbox, title, } = props;
    ...
    return (_jsx("iframe", { ref: iframeRef, src: attachUrl, title: title ?? 'Nimbus terminal', sandbox: sandbox ?? DEFAULT_SANDBOX, className: className, style: {
            ...
            border: 'none',
            ...style,
        }, "data-nimbus-tenant": tenant, "data-nimbus-sub": sub, "data-nimbus-session-id": sessionId }, renderKey));
});
