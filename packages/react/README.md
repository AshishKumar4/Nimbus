...
| `tenant` | `string` | ✓ | — | Embedder metadata; keep it aligned with the token's `tn` claim. The Worker trusts the token. |
| `sub` | `string` | | — | Embedder metadata; keep it aligned with the token's `sub` claim when present. |
