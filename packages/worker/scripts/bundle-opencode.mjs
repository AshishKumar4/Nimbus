...
async function emitGenerated(version, entryPath, sidecars, buildId, treeSitterWasms, tuiWorkers, yogaWasm, chunksPack) {
  const header = `/**
...
async function main() {
  ...
  await emitGenerated(
    ...
    resolveYogaWasm(sidecars),
    resolveChunksPack(sidecars),
  );
