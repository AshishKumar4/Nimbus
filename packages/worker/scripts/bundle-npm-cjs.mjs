 ...
 *
 * Output:
 *   - public/_assets/cirrus-npm-cjs-bundles.json containing the large
 *     prebuilt bundle map.
 *   - src/cirrus-npm-cjs.generated.ts containing only versions, asset path,
 *     and a typed async getter.
 */
...
const ASSET_FILENAME = 'cirrus-npm-cjs-bundles.json';
const ASSET_PATH = `/_assets/${ASSET_FILENAME}`;
...
async function main() {
  ...
  const assetJson = JSON.stringify(bundles);
  await fs.writeFile(path.join(ASSETS_DIR, ASSET_FILENAME), assetJson, 'utf8');

  // Emit the generated TS file. Shape:
  //   CIRRUS_NPM_CJS_VERSIONS: Record<pkgName, version>
  //   CIRRUS_NPM_CJS_BUNDLES_ASSET_PATH: string
  //   getCirrusNpmCjsBundles(env): Promise<Record<pkgEntrySpec, { moduleName, code }>>
  const header = `/**
...
export const CIRRUS_NPM_CJS_BUNDLES_ASSET_PATH = ${JSON.stringify(ASSET_PATH)};
...
`;
  ...
  console.log(
    `[bundle-npm-cjs] wrote ${OUT} (${(header.length / 1024).toFixed(1)} KB shim) + ` +
    `${path.join(ASSETS_DIR, ASSET_FILENAME)} (${(assetJson.length / 1024).toFixed(1)} KB)`,
  );
}
