      ...
      switch (sub) {
        ...
        case 'branch': {
          if (subArgs.length === 0 || subArgs[0] === '-a' || subArgs[0] === '--list') {
            ...
            const name = subArgs.find((a: string) => !a.startsWith('-'));
            if (name) {
            ...
            const flag = subArgs.find((a: string) => a.startsWith('-'));
            if (flag) {
        ...
        case 'checkout': {
          ...
          const checkoutFlag = subArgs.find((a: string) => a.startsWith('-') && a !== '-b');
          if (checkoutFlag) {
            ctx.stderr.write(`error: unknown switch \`${checkoutFlag}'\n`);
            ctx.stderr.write('usage: git checkout [-b] <branch>\n');
            return 129;
          }
          const ref = subArgs.find((a: string) => !a.startsWith('-'));
        ...
        case 'config': {
          ...
          for (const scope of subArgs.filter((a: string) => a.startsWith('-'))) {
          ...
          const key = subArgs.find((a: string) => !a.startsWith('-'));
