 ...
 */
export const NIMBUS_AI_CREDENTIAL_HEADERS = ['authorization', 'x-api-key'] as const;
 ...
 */
export function mintSessionAiToken(): string {
 ...
 */
export function requestCarriesSessionAiToken(headers: Headers, token: string): boolean {
  if (!token) return false;
  for (const name of NIMBUS_AI_CREDENTIAL_HEADERS) {
    const raw = headers.get(name);
    if (raw && presentedCredential(raw) === token) return true;
  }
  return false;
}
