...
function parseOptions(args: string[]): { ok: true; options: DdOptions } | { ok: false; error: string } {
  const options: DdOptions = { blockSize: 512, status: 'default' };

  for (const arg of args) {
    const operand = parseOperand(arg);
    if (!operand) return { ok: false, error: `unrecognized operand '${arg}'` };

    switch (operand.name) {
      case 'bs':
        options.blockSize = parsePositiveInteger(operand.value);
        if (options.blockSize <= 0) return { ok: false, error: `invalid block size '${operand.value}'` };
        break;
      case 'count': {
        const count = parseNonNegativeInteger(operand.value);
        if (count < 0) return { ok: false, error: `invalid count '${operand.value}'` };
        options.count = count;
        break;
      }
      case 'if':
      ...
      case 'of':
        ...
        break;
      case 'status':
        if (operand.value !== 'none') return { ok: false, error: `unsupported status '${operand.value}'` };
        options.status = 'none';
        break;
      default:
...
function parseOperand(arg: string): { name: string; value: string } | null {
...
async function readInput(
  ...
  if (options.input && options.input !== '/dev/stdin' && options.input !== '-') {
    if (options.input === '/dev/null') return '';
    if (options.input === '/dev/tty') {
      if (!ctx.terminalStdin) throw new Error('/dev/tty: no controlling terminal');
    ...
    try {
      ...
      if (error instanceof VFSError) throw new Error(`${options.input}: ${error.message}`);
      throw error;
    }
  }
...
async function readFromStream(stdin: CommandInputStream, options: DdOptions): Promise<string> {
  const limit = byteLimit(options);
  if (limit === undefined) return stdin.readAll();
  if (limit === 0) return '';
...
function limitContent(content: string, options: DdOptions): string {
  const limit = byteLimit(options);
  return limit === undefined ? content : content.slice(0, limit);
}

function byteLimit(options: DdOptions): number | undefined {
  return options.count === undefined ? undefined : options.count * options.blockSize;
}

function writeOutput(data: string, options: DdOptions, ctx: Parameters<Command>[0]): void {
  ...
  if (options.output === '/dev/null') return;
