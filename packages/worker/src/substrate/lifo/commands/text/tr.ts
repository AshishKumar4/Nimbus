...
function range(from: string, to: string): string {
  let out = '';
  for (let c = from.charCodeAt(0); c <= to.charCodeAt(0); c++) out += String.fromCharCode(c);
  return out;
}
...
function expandRange(set: string): string {
  const chars: { ch: string; literal: boolean }[] = [];
  for (let i = 0; i < set.length; i++) {
    if (set[i] === '[' && set[i + 1] === ':') {
      ...
      if (expansion !== undefined) {
        for (const ch of expansion) chars.push({ ch, literal: true });
        i = close + 1;
        continue;
      }
    }
