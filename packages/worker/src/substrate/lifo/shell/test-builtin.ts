...
export function evaluateTest(
  ...
  try {
    const result = parseExpr(args, 0, vfs, context);
...
function parseExpr(args: string[], pos: number, vfs: VFS, context: BuiltinExecutionContext | undefined): ExprResult {
  return parseOr(args, pos, vfs, context);
}

function parseOr(args: string[], pos: number, vfs: VFS, context: BuiltinExecutionContext | undefined): ExprResult {
  let left = parseAnd(args, pos, vfs, context);

  while (left.pos < args.length && args[left.pos] === '-o') {
    const right = parseAnd(args, left.pos + 1, vfs, context);
...
function parseAnd(args: string[], pos: number, vfs: VFS, context: BuiltinExecutionContext | undefined): ExprResult {
  let left = parsePrimary(args, pos, vfs, context);

  while (left.pos < args.length && args[left.pos] === '-a') {
    const right = parsePrimary(args, left.pos + 1, vfs, context);
    left = { value: left.value && right.value, pos: right.pos };
  }
...
function parsePrimary(args: string[], pos: number, vfs: VFS, context: BuiltinExecutionContext | undefined): ExprResult {
  if (pos >= args.length) {
  ...
  // Negation
  if (arg === '!') {
    const result = parsePrimary(args, pos + 1, vfs, context);
    return { value: !result.value, pos: result.pos };
  }

  // Parenthesized expression
  if (arg === '(') {
    const result = parseExpr(args, pos + 1, vfs, context);
  ...
  if (arg === '-t' && pos + 1 < args.length) {
    return { value: context?.isFdTerminal(toInt(args[pos + 1])) ?? false, pos: pos + 2 };
  }
