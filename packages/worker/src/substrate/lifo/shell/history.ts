...
function replaceOutsideHeredocBodies(input: string, search: string, replacement: string): string {
  const ranges = heredocBodyRanges(input);
  if (ranges.length === 0) return input.replaceAll(search, replacement);
...
function heredocBodyRanges(input: string): Array<{ start: number; end: number }> {
  return lex(input)
