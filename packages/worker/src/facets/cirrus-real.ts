...
export class CirrusReal {
  ...
  private _bootCompletedAt: number = 0;
  ...
  async start(ctx: DurableObjectState, pid: number): Promise<void> {
    ...
    const [realViteBundle, rollupWasmBase64, cirrusPluginReactBundle, cirrusNpmCjsBundles] = await Promise.all([
      ...
      getCirrusNpmCjsBundles(this.env as { ASSETS: { fetch: Fetcher['fetch'] } }),
    ]);
   ...
   * is not running.
   */
  async getDiag(): Promise<Record<string, unknown> | null> {
    if (!this.facetStub) return null;
    let facetMeta: any = null;
    try {
