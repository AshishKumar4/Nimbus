...
export class OnDemandBundleGate {
  ...
  private byteWaiters: Array<{ bytes: number; resolve: () => void }> = [];

  constructor(private readonly budgetBytes: number) {
    if (budgetBytes <= 0) throw new Error('OnDemandBundleGate budgetBytes must be > 0');
  }
   ...
   */
  async run<T>(job: (admit: (bytes: number) => Promise<void>) => Promise<T>): Promise<T> {
    // Acquire the build lock (serialize slice construction).
    const prev = this.buildLock;
    let releaseBuild!: () => void;
    this.buildLock = new Promise<void>((res) => { releaseBuild = res; });
    await prev;

    let admittedBytes = 0;
    let buildReleased = false;
    const releaseBuildOnce = () => {
      if (!buildReleased) { buildReleased = true; releaseBuild(); }
    };

    const admit = async (bytes: number): Promise<void> => {
    ...
    try {
      ...
      releaseBuildOnce();
      if (admittedBytes > 0) this.releaseBytes(admittedBytes);
    }
  }

  private acquireBytes(bytes: number): Promise<void> {
  ...
  get inFlightBytes(): number { return this.reservedBytes; }
}
