...
const SWAPS: ReadonlyArray<PackageSwapEntry> = [
  // Different-API candidates (bcrypt → bcryptjs, argon2 → hash-wasm,
  // node-sass → sass, grpc → @grpc/grpc-js, @swc/core → @swc/wasm-web)
  // are intentionally NOT here. A swap installs the target under the
  // original name, so `require(originalName)` would load a module whose
  // API the caller's code was not written against. They are in
  // `rejects` with a code-change suggestion.
  {
    from: 'esbuild',
    to: 'esbuild-wasm',
    reason:
...
const REJECTS: ReadonlyArray<PackageRejectEntry> = [
  ...
  // ── Rolldown (Vite 8's bundler): native shards plus a wasi-threads build ──
  {
    from: 'rolldown',
    reason:
      'Native Rust bundler that Vite 8 loads at startup. Its only non-native build, @rolldown/binding-wasm32-wasi, is a wasm32-wasip1-threads binary, and Workers run one thread per isolate with Atomics.wait disabled.',
    suggest:
      'no Workers-compatible target — rolldown publishes no single-threaded build, so tools that load Vite 8 themselves (Astro 7) cannot start here.',
    transitive: 'fail',
  },
  {
    from: '@rolldown/binding-wasm32-wasi',
    reason:
      'wasm32-wasip1-threads build of rolldown: it imports a shared memory and wasi thread-spawn, its Rust locks and thread parking execute memory.atomic.wait32, and its loader needs node:wasi and worker_threads. Workers run one thread per isolate with Atomics.wait disabled.',
 ...
 * against the target.
 */
export function swapAliasRange(swap: PackageSwapEntry, range: string): string {
  return `npm:${swap.to}@${range || 'latest'}`;
}
