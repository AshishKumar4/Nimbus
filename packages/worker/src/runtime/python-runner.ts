...
export function expandPythonEffectiveMode(effective: number): number {
...
export function installPythonFsSnapshot(
  ...
  const currentFiles = new Set(
    Object.keys(snapshot.modes || {}).filter((path) => !directories.has(path)),
  );

  for (const previous of previousFiles) {
    if (currentFiles.has(previous)) continue;
    const absolute = norm(previous);
    if (fs.analyzePath(absolute).exists) fs.unlink(absolute);
  }
  ...
  for (const directory of [...directories].sort((a, b) => b.length - a.length)) {
    ...
    if (mode !== undefined) fs.chmod(norm(directory), expandPythonEffectiveMode(mode));
  }
  return currentFiles;
}
...
interface PythonSocketProcessResult {
  exitCode: number;
  stdout: string;
  stderr: string;
  spawnedPid?: number;
