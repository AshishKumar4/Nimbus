...
export function rewriteCirrusViteConfigBundle(source: string): string {
...
function cirrusStaticSpecifier(specifier: string): string | undefined {
  if (specifier === 'vite' || specifier.startsWith('vite/')) return './vite-config-helper.js';
  if (specifier === '@vitejs/plugin-react' || specifier.startsWith('@vitejs/plugin-react/')) {
    return './cirrus-plugin-react.js';
  }
  if (specifier === 'node:fs') return './cirrus-fs.js';
  if (specifier === 'node:fs/promises') return './cirrus-fs-promises.js';
  return undefined;
}

function cirrusDynamicImport(specifier: string): string {
