 ...
 */
export function _rpcHostProcessProbe(_self: RpcHost): { isolateToken: string } {
...
export async function _rpcHostProcess(
  ...
  let settleHosted: (h: HostedResidentProcess) => void = () => {};
  let failHosted: (e: unknown) => void = () => {};
  const hosted = new Promise<HostedResidentProcess>((resolve, reject) => {
    settleHosted = resolve;
    failHosted = reject;
  });
  let settleBooted: (v: unknown) => void = () => {};
  let failBooted: (e: unknown) => void = () => {};
  const booted = new Promise<unknown>((resolve, reject) => {
 ...
 */
export function _rpcCancelHostProcess(self: RpcHost, workerKey: string): { cancelled: boolean } {
