 ...
 */
export async function _acquireOnDelivery(
  self: RpcHost,
  args: unknown,
  pid?: number,
): Promise<VfsDeliveredAcquire | undefined> {
 ...
 */
export function _acquireForRoutedRequest(self: RpcHost, pid: number): Promise<VfsDeliveredAcquire | undefined> {
  self.ensureSqliteFs();
  const vfs: SqliteVFS | null = self.sqliteFs;
  if (!vfs) return Promise.resolve(undefined);
 ...
 */
export async function _rpcFsReadBatch(
  self: RpcHost,
  requests: unknown,
  pid?: number,
...
export async function _rpcUnlink(self: RpcHost, path: string, pid?: number): Promise<void> {
   ...
   * }
   */
export async function _rpcWriteBatch(
  self: RpcHost,
  payload: unknown,
  pid?: number,
...
export async function _rpcRouteLoopback(self: RpcHost, port: number, request: Request): Promise<Response> {
    ...
    return res ?? new Response(
      JSON.stringify({ error: 'connection refused (no server listening)', port }),
 ...
 */
async function withDeliveredAcquire<T extends object>(
  self: RpcHost,
  reply: T,
  delivers: boolean,
  acquire: unknown,
...
export async function _rpcCpReadOutput(
  self: RpcHost,
  childPid: number,
  fd: 1 | 2,
  sinceSeq: number,
...
import { recordHit as _rpcRecordHit, recordMiss as _rpcRecordMiss, type CacheTier, type CacheKind } from '@nimbus-sh/core/_shared/cache-stats.js';
