...
import { clearPortCapability, type PortCapabilityHost } from './port-capability.js';

export interface ServingPortHost {
  ctx: PortCapabilityHost['ctx'];
  portRegistry: PortRegistry;
  facetManager: { registerPort(pid: number, port: number): Promise<void> } | null;
  ensureFacetManager?(): void;
}

export async function registerServingPort(self: ServingPortHost, pid: number, port: number): Promise<void> {
