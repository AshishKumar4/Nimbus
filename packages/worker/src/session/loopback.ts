 ...
 */
export async function routeSessionLoopback(
  self: LoopbackHost,
  port: number,
  request: Request,
): Promise<Response | null> {
  if (port === NIMBUS_AI_GATEWAY_PORT) return handleSessionAiRequest(self, request);
  if (!self.portRegistry.has(port)) return null;
