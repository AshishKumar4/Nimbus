 ...
 */
export function refusedNextSubcommand(
  scriptName: string,
  script: string,
  pkg: { dependencies?: Record<string, string>; devDependencies?: Record<string, string> },
  scriptArgs: readonly string[],
): 'dev' | 'start' | 'build' | 'export' | null {
  if (scriptArgs.includes('--force') || scriptArgs.includes('--allow-next')) return null;
  if (!(pkg.dependencies?.next || pkg.devDependencies?.next)) return null;
  if (scriptName === 'dev' || scriptName === 'start') return scriptName;
  ...
  while (i < tokens.length) {
    const t = tokens[i];
    if (/^[A-Z_][A-Z0-9_]*=/.test(t) || t === 'cross-env' || t === 'env' || t === 'npx') { i++; continue; }
    break;
  }
  const bin = (tokens[i] || '').replace(/^\.\/node_modules\/\.bin\//, '');
  if (bin !== 'next' && !bin.startsWith('next.')) return null;
  const sub = tokens[i + 1];
  if (sub === 'dev' || sub === 'start' || sub === 'build' || sub === 'export') return sub;
  return null;
}
 ...
 */
export const NEXT_REFUSAL_MESSAGE =
  '\x1b[31m✘\x1b[0m \x1b[1mNext.js is not supported in Nimbus.\x1b[0m\n' +
  '   Specific blockers:\n' +
 ...
 */
export const VITE_BUILD_TIMEOUT_MS = 120_000;
 ...
 */
export function withLoudTimeout<T>(
  work: Promise<T>,
  timeoutMs: number,
  what: string,
): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(new Error(
        `${what} produced no result after ${(timeoutMs / 1000).toFixed(0)}s — ` +
        'stopped waiting (the work may still finish in the background; re-run to retry).',
      ));
    }, timeoutMs);
    work.then(
