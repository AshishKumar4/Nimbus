...
export class NimbusSession extends CloudflareDurableObject {
    ...
    _notifySession(line) {
        ...
        try {
            appendScrollback(this.ctx, line, Date.now());
        }
        catch (e) {
            console.warn('[nimbus] session notice failed:', e?.message);
        }
    }
    /**
    ...
    async _rpcRemoveDurableApp(owner) {
        return _programmatic.rpcRemoveDurableApp(this, owner);
    }
    ...
    async _rpcDeleteFile(path, options, cred) { return _programmatic.rpcDeleteFile(this, path, options, cred); }
    /** Colocated embedders only (DO stub); not on the remote dispatcher. See `rpcSpawnWorker`. */
    async _rpcSpawnWorker(workerCode, command, cwd, opts) {
