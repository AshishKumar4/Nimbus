 ...
 */
export function handleFsWatchSubscribe(host, ws, msg) {
    const vfs = host.sqliteFs;
    if (!vfs)
        return { ok: false, error: 'VFS not initialised' };
    ...
    if (!host._fsWatchSubs)
        host._fsWatchSubs = new Map();
    ...
    const flush = () => {
        sub.flushTimer = null;
        if (sub.pending.length === 0 && sub.dropped === 0)
            return;
        ...
        sub.dropped = 0;
        try {
            const frame = { type: 'fs-watch-event', subId: sub.subId, events };
            if (dropped > 0)
     ...
     */
    const matches = (eventPath) => {
        if (paths.length === 0)
            return true;
        const ep = eventPath.replace(/^\/+/, '');
        for (const p of paths) {
            const norm = p.replace(/^\/+/, '');
            if (ep === norm || ep.startsWith(norm + '/'))
                return true;
        }
        return false;
    };
 ...
 */
export function handleFsWatchUnsubscribe(host, ws, msg) {
 ...
 */
export function cleanupFsWatchOnClose(host, ws) {
    const map = host._fsWatchSubs;
    if (!map)
        return;
    const list = map.get(ws);
 ...
 */
export function getFsWatchStats(host) {
    const map = host._fsWatchSubs;
    if (!map)
        return { wsCount: 0, subCount: 0, pendingTotal: 0, droppedTotal: 0 };
    let subCount = 0;
    let pendingTotal = 0;
    let droppedTotal = 0;
