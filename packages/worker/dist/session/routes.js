...
export async function handleFetch(self, request) {
    ...
    if (url.pathname === '/api/start-vite' && request.method === 'POST') {
        ...
        try {
            ...
            const apiViteEntry = self.processes.spawn('vite (api/start-vite, ' + root + ')', ['vite', '--port', String(apiVitePort)], root, { longRunning: true });
            self.viteDevServer = new ViteDevServer({
    ...
    if (url.pathname === '/api/_diag/abort' && request.method === 'POST') {
        ...
        const { promise: abortDue, resolve: abortReady } = withResolvers();
