...
import { parseViteConfigSource, unhandledVitePlugins, viteBuildBlockingPlugins } from '@nimbus-sh/core/runtime/vite-config-parser.js';
    ...
    registry.register('vite', async (ctx) => {
        ...
        if (args[0] === 'build') {
            ...
            if (buildSkippedPlugins.length) {
                ctx.stderr.write('\x1b[33m!\x1b[0m vite build: skipping plugins the built-in build cannot run' +
        ...
        if (devSkipped.length) {
            ctx.stderr.write('\x1b[33m!\x1b[0m vite: plugins are not evaluated by the built-in dev server' +
