...
import { ResidentCodeSpecSchema, facetImageDigest, facetImagePathDigest, } from '../loaders/process-fabric.js';
 ...
 */
async function readFacetImage(fs, path) {
...
export class NimbusLoadedWorker extends WorkerEntrypoint {
    ...
    getDurableObjectClass(name) {
        ...
        if (!outer)
            throw new Error('Nimbus: loaded worker code missing (key=' + propsAny.key + ')');
...
export class NimbusLoadedEntrypoint extends WorkerEntrypoint {
    ...
    async _codeWithSupervisor(props, includeSupervisor) {
