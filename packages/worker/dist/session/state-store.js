 ...
 */
export function ensureSessionStateSchema(ctx) {
    ...
    sql.exec('CREATE TABLE IF NOT EXISTS nimbus_session_kv (' +
        'k TEXT PRIMARY KEY, ' +
        'v TEXT NOT NULL)');
    sql.exec('CREATE TABLE IF NOT EXISTS nimbus_kernel_mounts (' +
        'mount_point TEXT PRIMARY KEY)');
    sql.exec('CREATE TABLE IF NOT EXISTS nimbus_terminal_scrollback (' +
    ...
    let hasBytes = false;
    for (const row of sql.exec('PRAGMA table_info(nimbus_terminal_scrollback)')) {
        if (String(row.name) === 'bytes') {
            hasBytes = true;
            break;
        }
    }
 ...
 */
export function loadShellState(ctx) {
    const sql = ctx?.storage?.sql;
    const empty = {
    ...
    let hydratedAt = null;
    let any = false;
    for (const row of sql.exec('SELECT k, v FROM nimbus_session_kv WHERE k IN (?, ?, ?)', KEY_CWD, KEY_ENV_JSON, KEY_HYDRATED_AT)) {
        any = true;
        if (row.k === KEY_CWD) {
            cwd = String(row.v);
        }
 ...
 * truncating.
 */
export function persistShellState(ctx, state) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return;
    ensureSessionStateSchema(ctx);
    if (state.cwd !== null) {
        sql.exec('INSERT OR REPLACE INTO nimbus_session_kv (k, v) VALUES (?, ?)', KEY_CWD, state.cwd);
    }
    else {
    ...
    if (state.env !== null) {
        ...
        if (json.length > SESSION_ENV_MAX_BYTES) {
            throw new Error(`[state-store] env JSON exceeds ${SESSION_ENV_MAX_BYTES} bytes ` +
                `(got ${json.length}); refusing to persist`);
        }
        sql.exec('INSERT OR REPLACE INTO nimbus_session_kv (k, v) VALUES (?, ?)', KEY_ENV_JSON, json);
    }
    else {
 ...
 */
export function stampHydratedAt(ctx, atMs) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return;
    ensureSessionStateSchema(ctx);
    sql.exec('INSERT OR REPLACE INTO nimbus_session_kv (k, v) VALUES (?, ?)', KEY_HYDRATED_AT, String(atMs));
}
/**
 ...
 * the cold-start path.
 */
export function clearSessionState(ctx) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return;
    ensureSessionStateSchema(ctx);
...
export function countSessionStateKeys(ctx) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return 0;
    ensureSessionStateSchema(ctx);
 ...
 */
export function loadKernelMounts(ctx) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return [];
    ensureSessionStateSchema(ctx);
    const out = [];
    for (const row of sql.exec('SELECT mount_point FROM nimbus_kernel_mounts ORDER BY mount_point ASC')) {
 ...
 */
export function persistKernelMounts(ctx, mounts) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return;
    ensureSessionStateSchema(ctx);
    ...
    for (const mp of mounts) {
        if (typeof mp !== 'string' || mp.length === 0)
            continue;
        sql.exec('INSERT OR REPLACE INTO nimbus_kernel_mounts (mount_point) VALUES (?)', mp);
    }
}
 ...
 */
export function appendScrollback(ctx, data, atMs) {
    ...
    try {
        sql.exec('INSERT INTO nimbus_terminal_scrollback (ts, data, bytes) VALUES (?, ?, ?)', atMs, payload, bytes);
        ...
        for (const row of sql.exec('SELECT last_insert_rowid() AS id')) {
            insertedSeq = Number(row.id);
            break;
        }
    }
    catch (e) {
        try {
            console.warn('[B\'.3] scrollback INSERT failed bytes=' + bytes + ' err=' + (e?.message || e));
        }
        catch { }
        return;
    }
    ...
    // two constants.
    let total = 0;
    for (const row of sql.exec('SELECT COALESCE(SUM(bytes), 0) AS n FROM nimbus_terminal_scrollback')) {
        total = Number(row.n) || 0;
        break;
    }
    if (total <= SCROLLBACK_MAX_BYTES)
        return;
    for (;;) {
        let oldestSeq = null;
        let oldestBytes = 0;
        for (const row of sql.exec('SELECT seq, bytes FROM nimbus_terminal_scrollback ORDER BY seq ASC LIMIT 1')) {
            oldestSeq = Number(row.seq);
            oldestBytes = Number(row.bytes) || 0;
            break;
        }
        if (oldestSeq === null)
            break;
 ...
 */
export function loadScrollback(ctx) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return '';
    ensureSessionStateSchema(ctx);
    const parts = [];
    for (const row of sql.exec('SELECT data FROM nimbus_terminal_scrollback ORDER BY seq ASC')) {
        parts.push(String(row.data));
    }
    return parts.join('');
}
/** Stats for /api/_diag/session: row count + total bytes + cap. */
export function getScrollbackStats(ctx) {
    const sql = ctx?.storage?.sql;
    if (!sql)
        return { rows: 0, bytes: 0, maxBytes: SCROLLBACK_MAX_BYTES };
    ensureSessionStateSchema(ctx);
    let rows = 0;
    let bytes = 0;
    for (const row of sql.exec('SELECT COUNT(*) AS n, COALESCE(SUM(bytes), 0) AS b FROM nimbus_terminal_scrollback')) {
        rows = Number(row.n) || 0;
