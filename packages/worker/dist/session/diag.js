 ...
 */
import { rehydrateFromStorage, snapshotForStorage, getFailures, } from '../observability/oom-discriminator.js';
 ...
 */
export function sampleMemory(host) {
 ...
 */
export async function rehydrateRingFromStorage(_host, ctx) {
    try {
        const blob = await ctx.storage.get(W5_RING_STORAGE_KEY);
        if (blob)
            rehydrateFromStorage(blob);
    }
    catch (_e) {
 ...
 */
export function persistRing(host, ctx) {
    try {
        const failures = getFailures();
        if (failures.length === 0)
            return null;
        if (failures.length === host._w5LastPersistRingSize)
            return null;
        ...
        return ctx.storage.put(W5_RING_STORAGE_KEY, snap).catch((e) => {
