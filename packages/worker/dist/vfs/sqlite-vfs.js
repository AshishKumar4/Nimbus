...
export class SqliteVFS {
    ...
    as(cred) {
        ...
        return {
            ...
            appendOnce: (path, pid, writerId, operationId, digest, bytes) => (this.appendOnce(path, pid, writerId, operationId, digest, bytes, bound)),
            acknowledgeAppend: (pid, writerId, operationId) => (this.acknowledgeAppend(pid, writerId, operationId)),
     ...
     */
    appendOnce(path, pid, writerId, operationId, digest, bytes, cred) {
        ...
        const normalized = normalizeVfsPath(path);
        let writer = [...this.sql.exec(`SELECT acked_through, revoked
       ...
       WHERE pid = ? AND writer_id = ?`, pid, writerId)][0];
        if (!writer) {
         ...
         (pid, writer_id, acked_through, revoked) VALUES (?, ?, 0, 0)`, pid, writerId);
       ...
       WHERE pid = ? AND writer_id = ? AND operation_id = ?`, pid, writerId, operationId)][0];
        if (acknowledgedGap) {
       ...
       WHERE pid = ? AND writer_id = ? AND operation_id = ?`, pid, writerId, operationId)][0];
        ...
        const highestKnown = Number([...this.sql.exec(`SELECT MAX(operation_id) AS operation_id
         ...
         )`, pid, writerId, pid, writerId)][0]?.operation_id
            ?? ackedThrough);
        ...
        const recordReceipt = () => {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`, pid, writerId, operationId, normalized, bytes.byteLength, digest, Date.now());
        };
    ...
    acknowledgeAppend(pid, writerId, operationId) {
        ...
        const writer = [...this.sql.exec(`SELECT acked_through, revoked FROM vfs_append_writer_state
       WHERE pid = ? AND writer_id = ?`, pid, writerId)][0];
       ...
       WHERE pid = ? AND writer_id = ? AND operation_id = ?`, pid, writerId, operationId)].length > 0;
        if (existingGap)
            return;
        const receipt = [...this.sql.exec(`SELECT path, byte_length, digest
       FROM vfs_append_receipts
       WHERE pid = ? AND writer_id = ? AND operation_id = ?`, pid, writerId, operationId)][0];
        if (!receipt) {
        ...
        this.transactionSync(() => {
         ...
         VALUES (?, ?, ?, ?, ?, ?)`, pid, writerId, operationId, receipt.path, receipt.byte_length, receipt.digest);
            this.sql.exec(`DELETE FROM vfs_append_receipts
         WHERE pid = ? AND writer_id = ? AND operation_id = ?`, pid, writerId, operationId);
            const acknowledged = [...this.sql.exec(`SELECT operation_id FROM vfs_append_acked_gaps
         WHERE pid = ? AND writer_id = ? AND operation_id > ?
         ORDER BY operation_id`, pid, writerId, ackedThrough)];
            ...
            this.sql.exec(`DELETE FROM vfs_append_acked_gaps
         WHERE pid = ? AND writer_id = ? AND operation_id <= ?`, pid, writerId, ackedThrough);
            this.sql.exec(`UPDATE vfs_append_writer_state SET acked_through = ?
         WHERE pid = ? AND writer_id = ?`, ackedThrough, pid, writerId);
        });
    }
    revokeAppendWriter(pid, writerId) {
        if (!Number.isSafeInteger(pid) || pid <= 0)
            throw vfsError('EINVAL', `invalid append pid ${pid}`);
        this.transactionSync(() => {
            this.sql.exec(`UPDATE vfs_append_writer_state SET revoked = 1
         WHERE pid = ? AND writer_id = ?`, pid, writerId);
            this.sql.exec('DELETE FROM vfs_append_receipts WHERE pid = ? AND writer_id = ?', pid, writerId);
            this.sql.exec('DELETE FROM vfs_append_acked_gaps WHERE pid = ? AND writer_id = ?', pid, writerId);
        });
    }
