 ...
 */
export function buildFsSnapshot(vfs, projectRoot) {
    const files = {};
    ...
    function walkUserTree(relDir, depth) {
        if (depth > 32)
            return;
        if (totalBytes >= MAX_TOTAL_USER_BYTES)
            return;
        let entries;
        try {
            entries = vfs.readdir(relDir);
        }
        catch {
            return;
        }
        ...
        for (const entry of entries) {
            ...
            if (entry.type === 'directory') {
                if (SKIP_DIRS_IN_USER_PROJECT.has(entry.name))
                    continue;
                walkUserTree(childRel, depth + 1);
            }
            else if (entry.type === 'file') {
                const isPkgJson = entry.name === 'package.json';
                if (!isPkgJson && !isLikelyText(entry.name)) {
                    skipped++;
                    continue;
                }
                if (totalBytes >= MAX_TOTAL_USER_BYTES) {
                ...
                try {
                    ...
                    if (st.size > MAX_FILE_BYTES && !isPkgJson) {
                        skipped++;
                        continue;
                    }
    ...
    // bounded.
    const entryFilesToSeed = new Set();
    function collectExportPaths(node, out) {
        ...
        if (Array.isArray(node)) {
            for (const n of node)
                collectExportPaths(n, out);
            return;
        }
        if (typeof node === 'object') {
            for (const k of Object.keys(node)) {
                collectExportPaths(node[k], out);
            }
        }
    }
    function walkNodeModules(relDir, depth) {
        if (depth > 32)
            return;
        let entries;
        try {
                ...
                if (entry.name === 'package.json') {
                    try {
                        ...
                        if (typeof pkg.module === 'string')
                            candidates.add(pkg.module);
                        if (typeof pkg.browser === 'string')
                            candidates.add(pkg.browser);
                        if (typeof pkg.types === 'string')
                            candidates.add(pkg.types);
                        if (typeof pkg.typings === 'string')
                            candidates.add(pkg.typings);
                        if (pkg.exports)
                            collectExportPaths(pkg.exports, candidates);
                        if (pkg.imports)
                            collectExportPaths(pkg.imports, candidates);
    ...
    if (vfs.exists(nmPath))
        walkNodeModules(nmPath, 0);
 ...
 */
export function generateFsShimModuleCode() {
 ...
 */
export function generateFsPromisesShimModuleCode() {
    return `
...
export function generateSyntheticModuleCode(opts) {
    const { viteVersion, snapshotFiles, snapshotDirs, existingPaths, rollupWasmBase64, cjsPrebuiltBundles, viteClientMjs, viteEnvMjs, } = opts;
    ...
    // didn't cover.
    const allFiles = { ...viteSynthetics, ...snapshotFiles };
...
${body}
  ...
  existingPaths: new Set(${existingPaths && existingPaths.length > 0
...
${Object.entries(cjsPrebuiltBundles).map(([pathPattern, code]) => `  [${JSON.stringify(pathPattern)}, ${JSON.stringify(code)}]`).join(',\n')}
]);
