 ...
 */
export const PACKAGE_ABI_POLICY = {
...
// ─────────────────────────────────────────────────────────────────────────
export function policyLookupSwap(policy, name) {
 ...
 * against the target.
 */
export function swapAliasRange(swap, range) {
 ...
 */
export function policyIsOptionalNativeBinding(policy, p) {
    if (!p)
        return false;
...
export function policyNativePlatformReject(policy, pkg) {
    ...
    if (os.length > 0 || cpu.length > 0 || libc.length > 0) {
        ...
        return {
            from: pkg.name,
            reason: `Package ${pkg.name} only ships platform-native artifacts (${constraints}; ` +
                `artifact class '${policy.nativeArtifactClass}'). ` +
                'Nimbus cannot execute Linux/Windows/macOS native binaries; ' +
                `publish a JavaScript, WASM, or ${policy.abiTarget} artifact.`,
            transitive: 'fail',
...
(() => {
    for (const s of PACKAGE_ABI_POLICY.swaps) {
        if (PACKAGE_ABI_POLICY.rejects.some((r) => r.from === s.from)) {
            throw new Error(`Registry conflict: '${s.from}' is in both swaps and rejects. ` +
