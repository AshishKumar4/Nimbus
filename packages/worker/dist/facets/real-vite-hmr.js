...
export class HmrBridge {
    ...
    relayToBrowser(id, msg) {
        ...
        if (!ws) {
            if (globalThis.__cirrusDebug)
                console.log('[HmrBridge relay] no WS for id', id);
            return;
        }
        try {
            ws.send(msg);
            if (globalThis.__cirrusDebug)
                console.log('[HmrBridge relay] sent ok');
        }
        catch (e) {
            if (globalThis.__cirrusDebug)
                console.log('[HmrBridge relay] send threw:', e?.message);
        }
    }
    /**
     ...
     */
    pushVfsEvent(event, path, oldPath) {
     ...
     */
    async nextEvents(timeoutMs = 25_000) {
...
export class CirrusHmrRPC extends WorkerEntrypoint {
    ...
    async hmrSend(clientId, msg) {
        ...
        // sockets.
        try {
            await useRpcResource(resolved.dispatch({ op: 'hmrRelay', args: [clientId, msg] }), () => undefined);
        }
        catch {
            /* socket gone */
        }
 ...
 */
export function generateWsShimModuleCode() {
    return `
 ...
 */
export function generateChokidarShimModuleCode() {
    return `
