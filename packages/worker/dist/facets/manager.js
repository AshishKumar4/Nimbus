...
export class FacetManager {
    ...
    async execStagedArtifact(artifact, opts) {
        ...
        const workerConfig = staged.supervisorBinding
            ? { ...staged.baseConfig, env: { SUPERVISOR: staged.supervisorBinding } }
            : staged.baseConfig;
        if (mode === 'attached') {
            return await this._execStagedArtifactAttached(staged.pid, staged.command, workerConfig, staged.supervisorBinding);
        }
        let worker;
        let entrypoint;
        try {
    ...
    async _runOpencodeServerFacet(staged, port) {
        ...
        const workerConfig = supervisorBinding
        ...
        try {
            ...
            this.trackProcessRpcResources(pid, [routeStub, startStub, worker, supervisorBinding], { releaseOnReportExit: false });
     ...
     */
    async _awaitOpencodeServerReady(pid, port, timeoutMs = 20000) {
        ...
        while (Date.now() < deadline) {
            ...
            if (this.portRegistry.has(port)) {
                try {
                    const res = await this.portRegistry.routeRequest(port, new Request(`http://127.0.0.1:${port}/doc`), '/doc');
