...
function generateMainModuleCode(opts) {
    ...
    const configImportLine = hasUserViteConfig
        ...
        : `const __userConfig = {};`;
    return `
// ── Facet entrypoint: real Vite in a DO Facet [D'.1] ───────────────
// Generated by src/cirrus-real.ts.
//
// Track D' / D'.1: this module exports a DurableObject class so the
// supervisor can spawn it via ctx.facets.get('cirrus-real-vite',
// {class}). The facet has its own SQLite (this.ctx.storage) which we
// use to persist a per-facet identity cookie — that's what proves
// the same facet instance is reused across supervisor reconnects vs.
// being re-created from scratch.
//
// Pre-D'.1 the export was \`export default { fetch(req, env, ctx) }\`
// — a stateless WorkerEntrypoint shape spawned via env.LOADER.load().
// That worker had no own storage and was rebuilt on every start().
...
// ── DurableObject class — the D'.1 entrypoint ───────────────────────
//
// Lifecycle:
//   - First ctx.facets.get(...) call: workerd instantiates this class
//     ONCE per facet name. The constructor runs; this.ctx.storage is
//     a fresh empty SQLite. ensureFacetCookie() writes a UUID row.
//   - Subsequent ctx.facets.get(...) calls in the same supervisor
//     isolate (or after hibernation/wake): workerd reuses the same
//     instance; storage rows survive. ensureFacetCookie() reads
//     the existing UUID.
//
// The supervisor's CirrusReal controller calls fetch() for every
// preview request and getDiag() for the /api/_diag/cirrus surface.
// HMR pump scheduling now lives on this class (each fetch() kicks
// pumpHmrOnce via this.ctx.waitUntil).
export class CirrusRealVite extends DurableObject {
  ...
  constructor(state, env) {
    super(state, env);
    this.__bootStartedAt = Date.now();
    this.__bootCompletedAt = 0;
    this.__cookie = '';
    // Ensure schema + cookie row up front. SQLite operations on the
    // facet's own storage; survives hibernation/wake of THIS facet.
    try {
      const sql = state?.storage?.sql;
      if (sql) {
        sql.exec('CREATE TABLE IF NOT EXISTS cirrus_facet_meta (k TEXT PRIMARY KEY, v TEXT NOT NULL)');
        let found = '';
        for (const row of sql.exec('SELECT v FROM cirrus_facet_meta WHERE k = ?', 'cookie')) {
          found = String(row.v);
          break;
        }
        if (found) {
          ...
          // Mint a fresh UUID-shaped cookie and persist. crypto.randomUUID
          // is available in workerd globals.
          const c = (globalThis.crypto?.randomUUID?.() || ('cookie-' + Math.random().toString(16).slice(2)));
          sql.exec('INSERT OR REPLACE INTO cirrus_facet_meta (k, v) VALUES (?, ?)', 'cookie', c);
          this.__cookie = c;
        }
      }
    } catch (e) {
      console.warn('[cirrus-real-facet] cookie-init failed:', e?.message || e);
    }
  }

  /** RPC: identity + boot stats so the supervisor can surface them
   *  on /api/_diag/cirrus. */
  async getFacetMeta() {
    return {
      cookie: this.__cookie,
      bootMs: this.__bootCompletedAt > 0
        ? this.__bootCompletedAt - this.__bootStartedAt
        : 0,
...
export class CirrusReal {
    ...
    async start(ctx, pid) {
        ...
        //   its own hibernation lifecycle, and is owned by THIS supervisor
        ...
        try {
            ...
            if (CirrusRealViteClass && typeof ctx.facets?.get === 'function') {
                // PRIMARY: DO Facet path.
                this._facetName = facetName;
                this.facetStub = ctx.facets.get(facetName, async () => ({
    ...
    stop(ctx) {
        // [D'.1] Tear down the DO Facet via ctx.facets.delete so its own
        // SQLite storage is reclaimed. Pre-D'.1 the worker stub was
        // garbage-collected when facetStub was nulled — facets need an
    ...
    deliverHmrClientMessage(id, msg) {
    ...
    async handleRequest(request, pathname) {
        if (this.bootError) {
            return new Response('[cirrus-real] facet spawn failed:\n' + this.bootError, { status: 500, headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
        }
        if (!this.facetStub) {
        ...
        try {
            ...
            this._lastFetchMs = Date.now() - fetchT0;
            ...
            return new Response('[cirrus-real] facet.fetch threw: ' + (e?.stack || e?.message || String(e)), { status: 502, headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
        }
    }
    /**
    ...
    async getDiag() {
        ...
        let facetMeta = null;
        try {
            facetMeta = await this.facetStub.getFacetMeta();
        }
        catch (e) {
            facetMeta = { rpcError: e?.message || String(e) };
        }
        return {
            kind: this._kind,
            facetName: this._facetName,
            cookie: facetMeta?.cookie ?? null,
            bootMs: this._startedAt > 0 ? this._bootCompletedAt - this._startedAt : 0,
            facetBootMs: facetMeta?.bootMs ?? 0,
            lastFetchMs: this._lastFetchMs,
            bootError: this.bootError,
            facetBootError: facetMeta?.bootError ?? null,
            viteServerListening: facetMeta?.viteServerListening ?? null,
            uptimeMs: this._startedAt ? Date.now() - this._startedAt : 0,
            hmrClients: this.hmr.size,
        };
    }
