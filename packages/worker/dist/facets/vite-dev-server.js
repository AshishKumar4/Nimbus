...
import { rewriteJavaScriptModuleSource } from '../runtime/module-source-rewriter.js';
 ...
 */
export function rewriteAllImports(code, aliases, basePath, importerCtx) {
...
function cssSideEffectModuleSpecifier(specifier, context) {
    if (!context.isSideEffectImport)
        return undefined;
    const parts = splitSpecifierSuffix(specifier);
    if (!parts.pathname.endsWith('.css'))
        return undefined;
    if (!parts.suffix)
        return `${parts.pathname}?import`;
    if (parts.suffix.startsWith('#'))
        return `${parts.pathname}?import${parts.suffix}`;
    return specifier;
}
...
export class ViteDevServer {
    ...
    basePath;
    running = false;
    moduleCache = new LruMap(VITE_MODULE_CACHE_MAX_ENTRIES);
    unsubVfs = null;
    ...
    handleVfsEvents(events) {
        ...
        for (const event of events) {
            ...
            const pathVariants = new Set([path, this.root + '/' + path]);
     ...
     */
    async handleRequest(request, pathname, mountBase) {
        const t0 = Date.now();
    ...
    async _handleRequestInner(request, pathname, base) {
    ...
    async serveModuleCold(specifier, headers, base, knownBarrelInfo = null, admit) {
        ...
        if (resolved) {
            ...
            if (isBarrel) {
                ...
                if (!synth) {
                    // barrel fallback support: emit an honest 200 stub instead of a
                ...
                try {
                    ...
                    // barrel fallback support: same 200-stub pattern as the synth-null
    ...
    async serveFile(request, pathname, query, headers, base) {
