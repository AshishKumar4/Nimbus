 ...
 */
export const NIMBUS_AI_CREDENTIAL_HEADERS = ['authorization', 'x-api-key'];
/**
 ...
 */
export function requestCarriesSessionAiToken(headers, token) {
    if (!token)
        return false;
    for (const name of NIMBUS_AI_CREDENTIAL_HEADERS) {
        const raw = headers.get(name);
        if (raw && presentedCredential(raw) === token)
            return true;
    }
    return false;
}
