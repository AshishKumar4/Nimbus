 ...
 */
export function resolveExports(exportsField, subpath = '.', conditions = DEFAULT_ESM_CONDITIONS) {
    ...
    if (isSubpathMap) {
        ...
        for (const pattern of wildcardKeys) {
            ...
            if (subpath.startsWith(prefix) &&
                ...
                if (target === null)
                    return null;
                const matched = subpath.slice(prefix.length, suffix ? subpath.length - suffix.length : undefined);
                const resolved = resolveConditionValue(target, conditions);
                if (resolved)
                    return resolved.split('*').join(matched);
            }
        }
        return null;
    }
 ...
 */
export function resolvePackageEntry(pkg, subpath = '.', conditions = DEFAULT_ESM_CONDITIONS) {
    // 1. exports field
 ...
 */
export function getExportsResolverJS() {
    return `
