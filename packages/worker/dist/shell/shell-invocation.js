...
export function parseShellInvocation(shellName, rawArgs) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '-o') {
            if (args[i + 1] === undefined)
                return error(shellName, '-o requires an argument', 2);
        ...
        if (LONG_MODE_FLAGS.has(arg))
            continue;
        if (arg.startsWith('--'))
            return error(shellName, `unsupported option: ${arg}`, 2);
...
function parseOptionCluster(shellName, args, index, options) {
    ...
    for (let i = 1; i < cluster.length; i++) {
        ...
        if (flag === 'c') {
            ...
            if (parsed.ok)
                return { ok: true, consumed: 1, invocation: parsed.invocation };
            return { ok: false, error: parsed.error, exitCode: parsed.exitCode };
        }
        if (flag === 's')
            return { ok: true, consumed: 0, invocation: { kind: 'stdin', args: [], options: { ...options } } };
...
function commandFromNextArg(shellName, args, index, options) {
    const command = args[index + 1];
    if (command === undefined)
        return error(shellName, '-c requires an argument', 2);
    return {
        ok: true,
        invocation: { kind: 'command', body: command, args: args.slice(index + 2), options: { ...options } },
    };
}
function error(shellName, message, exitCode) {
    return { ok: false, error: `${shellName}: ${message}`, exitCode };
}
function isOptionCluster(arg) {
    return arg.length > 2 && arg[0] === '-' && arg !== '--';
}
function applyShellFlag(flag, enabled, options) {
    if (flag === 'e')
        options.errexit = enabled;
    if (flag === 'u')
        options.nounset = enabled;
}
function applyShellOption(_shellName, option, enabled, options) {
    switch (option) {
        case 'errexit':
            options.errexit = enabled;
            return true;
