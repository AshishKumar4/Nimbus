 ...
 */
export function expandGlobs(tokens, cwd, vfs) {
    const result = [];
 ...
 */
export function parseRedirects(cmd) {
    let redirectOut;
    let redirectAppend;
    let redirectErr;
 ...
 */
export function parseHeredoc(input) {
    ...
    if (rest.startsWith("'")) {
        ...
        if (!match)
            return null; // no valid delimiter
        delimiter = match[1];
        quoted = false;
    }
    if (!delimiter)
...
export class HeredocHandler {
    shell; // Shell instance (from @lifo-sh/core)
    terminal; // ITerminal-compatible (has write())
    vfs; // SqliteVFS
    originalExecuteLine = null;
    originalPrintPrompt = null;
    ...
    constructor(shell, terminal, vfs) {
        this.shell = shell;
     ...
     */
    static install(shell, terminal, vfs) {
    ...
    async _finishHeredoc() {
        ...
        // shell compatibility (2026-05-11): expand $NAME / ${NAME} in heredoc
        ...
        try {
            if (info.redirectFile) {
                ...
                await this.shell.execute(info.command, {
                    stdin: content,
                    onStdout: (d) => this.terminal.write(d),
                    onStderr: (d) => this.terminal.write(d),
                });
            }
        }
        catch (e) {
...
export class SubshellNormalizer {
        ...
        this.shell.executeLine = async (line) => {
            ...
            if (groups.length === 1 && groups[0].start === line.indexOf('(') &&
                ...
                try {
                    ...
                    if (typeof shell.setCwd === 'function')
                        shell.setCwd(savedCwd);
                    else if ('cwd' in shell)
                        shell.cwd = savedCwd;
     ...
     */
    static findTopLevelGroups(line) {
        const groups = [];
        let depth = 0;
        let groupStart = -1;
        let i = 0;
...
function tokenizeRespectingQuotes(line) {
    const tokens = [];
    let cur = '';
    let i = 0;
    while (i < line.length) {
 ...
 */
function expandTokenBraces(tok) {
...
export class DollarVarShim {
     ...
     */
    static expandDollarVars(line, pid) {
        let out = '';
        let i = 0;
        while (i < line.length) {
            const ch = line[i];
 ...
 */
export function expandHeredocVars(content, env) {
    let out = '';
    let i = 0;
...
export class LineEditorExtender {
    ...
    _handleBinding(data) {
        ...
        switch (data) {
            ...
            case '\x17': {
                if (s.cursorPos === 0)
                    return true;
                const buf = s.lineBuffer;
                let i = s.cursorPos;
                // Skip trailing whitespace BEFORE the cursor.
                while (i > 0 && /\s/.test(buf[i - 1]))
                    i--;
    ...
    _prevWordStart(buf, pos) {
        let i = pos;
        // Skip non-word chars (whitespace, punctuation) backward.
        while (i > 0 && !LineEditorExtender.isWordChar(buf[i - 1]))
            i--;
    ...
    _nextWordEnd(buf, pos) {
        let i = pos;
        // Skip non-word chars forward.
        while (i < buf.length && !LineEditorExtender.isWordChar(buf[i]))
            i++;
    ...
    _caseTransformWord(mode) {
        ...
        if (wordEnd === wordStart)
            return true; // no word to transform
    ...
    _rsearchHandle(data) {
        ...
        if (data === '\x03') {
            ...
            if (typeof s.printPrompt === 'function')
                s.printPrompt();
            return;
        }
 ...
 */
export async function executeCommandLine(input, ctx) {
