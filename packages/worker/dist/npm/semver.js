...
export function parseSemver(v) {
    ...
    if (!m)
        return null;
    const prerelease = m[4] === undefined
        ? []
 ...
 */
export function semverComparators(part) {
    const comp = part.trim();
    if (comp === '' || comp === '*' || comp === 'x' || comp === 'X' || comp === 'latest')
        return [];
    ...
    if (patchWild) {
        ...
        if (op === '^')
            return upto(major === 0 ? [0, minor + 1, 0, []] : [major + 1, 0, 0, []]);
...
export function satisfiesRange(version, range) {
    ...
    for (const orPart of trimmed.split(/\s*\|\|\s*/)) {
        ...
        if (!ok)
            continue;
        if (v[3].length > 0) {
            const admitted = comparators.some(([, r]) => r[3].length > 0 && r[0] === v[0] && r[1] === v[1] && r[2] === v[2]);
            if (!admitted)
                continue;
        }
        return true;
...
export function isSemverRange(range) {
    ...
    for (const orPart of trimmed.split(/\s*\|\|\s*/)) {
        ...
        for (const part of orPart.trim().split(/\s+/)) {
            if (semverComparators(part) === null)
                return false;
        }
    }
    return true;
...
export function resolveVersion(versions, range) {
    ...
    for (const v of versions) {
        if (!satisfiesRange(v, trimmed))
            continue;
        const p = parseSemver(v);
        if (!p)
            continue;
        if (best === null || compareSemver(p, best.p) > 0)
            best = { v, p };
    }
