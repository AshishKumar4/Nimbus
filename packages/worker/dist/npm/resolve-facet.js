...
export const resolveTreeInFacet = async function resolveTreeInFacet(spec, env) {
    const t0 = Date.now();
    const messages = [];
    ...
    for (const entry of spec.cachedEntries || []) {
        if (!entry || !entry.name || !entry.version)
            continue;
    ...
    const flushCache = async () => {
        ...
        pendingCacheWrites = [];
        try {
            await __nimbusUseRpcResult(env.SUPERVISOR.putRegistryEntries(batch), (r) => {
    ...
    const cachedEntryToResolved = (entry) => {
        let deps = {}, peers = {}, exp = null, bin = {};
        let platform = {}, optionalDeps = {};
        try {
            deps = JSON.parse(entry.depsJson);
        }
        catch { }
        try {
            peers = entry.peerDepsJson ? JSON.parse(entry.peerDepsJson) : {};
        }
        catch { }
        try {
            exp = JSON.parse(entry.exportsJson);
        }
        catch { }
        try {
            bin = JSON.parse(entry.binJson);
        }
        catch { }
        try {
            platform = entry.platformJson ? JSON.parse(entry.platformJson) : {};
        }
        catch { }
        try {
            optionalDeps = entry.optionalDepsJson ? JSON.parse(entry.optionalDepsJson) : {};
        }
        catch { }
        return {
            name: entry.name,
    ...
    const fetchPackumentWithRetry = async (name) => {
        ...
        if (r2Available) {
            try {
                const r2P = Promise.race([
                    __nimbusUseRpcResult(env.SUPERVISOR.getCachedPackument(name), (result) => result),
                    new Promise((rs) => setTimeout(() => rs(null), R2_PACKUMENT_RACE_TIMEOUT_MS)),
                ]).catch(() => null);
                ...
                try {
                    if (resp.ok) {
                        ...
                        if (typeof env.SUPERVISOR.putCachedPackument === 'function') {
                            try {
                                await __nimbusUseRpcResult(env.SUPERVISOR.putCachedPackument(name, text), () => undefined);
                            }
                            catch {
    ...
    const resolveFromCache = (name, range) => {
        ...
        if (cleanRange.match(/^\d+\.\d+\.\d+$/)) {
            ...
            if (exact)
                return cachedEntryToResolved(exact);
        }
    ...
    const topLevelNames = new Set(Object.keys(spec.specs));
    const handleNativeExecutableReject = (requestName, pkg, opts) => {
    ...
    const registryRejectError = (from, reason) => Object.assign(new Error(`npm install rejected: ${from} — ${reason}`), {
    ...
    const resolveOne = async (name, range, opts = {}) => {
        // @ts-ignore — SHOULD_SKIP_PACKAGE provided by preamble.
        if (!topLevelNames.has(name) && SHOULD_SKIP_PACKAGE(name, !!spec.frameworkAware))
            return null;
        // Registry transitive policy. Swaps rewrite names in flight; 'fail'
        // rejects throw; 'warn' rejects log [skip] and drop. Decisions are also
        // pushed into the preamble-provided event buffer, which the supervisor
        // drains after successful facet returns.
        let effName = name;
        ...
        if (cached) {
            if (handleNativeExecutableReject(name, cached, opts))
                return null;
            return cached;
        }
        ...
        if (handleNativeExecutableReject(name, pkg, opts))
            return null;
        ...
        for (let i = 0; i < Math.min(5, sorted.length); i++) {
            ...
            try {
                ...
                if (!inner) {
                    inner = new Map();
                    cacheByName.set(otherPkg.name, inner);
                }
    ...
    const queue2 = Object.entries(spec.specs);
    ...
    const __f2Diag = !!spec.__f2DiagWidths;
    let __f2LayerN = 0;
    while (queue2.length > 0) {
        ...
        const results = await Promise.all(batch.map(([name, range]) => limit(async () => {
            ...
            try {
                ...
                // REJECT_INSTALL with transitive='fail' throws from resolveOne
                // with an own-property tag. Propagate those; turning them into
                // "UNHANDLED" log lines would silently partial-install.
                if (e && typeof e === 'object' && e.__nimbus_registry_reject === true) {
        ...
        for (const pkg of results) {
            ...
            if (pkg.peerDependencies) {
                for (const [peerName, peerRange] of Object.entries(pkg.peerDependencies)) {
                    ...
                    queue2.push([peerName, peerRange]);
                }
            }
