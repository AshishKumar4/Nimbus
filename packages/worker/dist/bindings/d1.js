...
class TablePrefixer {
    ...
    prefixed(rawName) { return this.prefix + rawName; }
    has(rawName) { return this.known.has(rawName.toLowerCase()); }
    _rewrite(sql) {
        ...
        const isWord = (k, w) => k >= 0 && k < tokens.length && tokens[k].type === 'word' &&
            stripQuoting(tokens[k].value).toUpperCase() === w;
        const rewriteAtIfKnown = (k, registerNew = false) => {
...
export class D1Emulator {
    ...
    async batch(stmts) {
        ...
        txn(() => {
            for (const s of stmts) {
                const r = this._runOneSync(s._sql, s._params);
                if (!r.success)
                    throw new Error(r.error || 'batch failed');
                results.push(r);
            }
        });
        return results;
    ...
    _meta(rowsRead, rowsWritten, changes, lastRowId, t0) {
        return {
    ...
    async applyMigrations() {
        if (this.migrationsRun || !this.migrationsDir || !this.vfs)
            return { applied: 0 };
