 ...
 */
export function rewriteLocationForOuter(location, outerWorkerBase, currentRequestUrl) {
    if (!location)
        return location;
    ...
    try {
        ...
        if (loc.origin !== current.origin)
            return location;
        if (loc.pathname.startsWith(base + '/') || loc.pathname === base)
            return location;
 ...
 */
function renderWorkerRunningHtml(opts) {
...
export class NimbusWrangler {
    ...
    stop() {
        ...
        if (this.doFacetNames.size > 0 && this.supervisorCtx?.facets?.abort) {
            for (const name of this.doFacetNames) {
                try {
                    this.supervisorCtx.facets.abort(name, new Error('nimbus-wrangler stopped'));
                }
                catch { }
            }
        }
    ...
    async buildAndLoad() {
        ...
        try {
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => {
                    timeoutHandle = setTimeout(() => {
                        reject(new Error(`esbuild bundle exceeded ${BUILD_TIMEOUT_MS / 1000}s timeout. ` +
            ...
            if (this.doFacetNames.size > 0 && this.supervisorCtx?.facets?.abort) {
                for (const name of this.doFacetNames) {
                    try {
                        this.supervisorCtx.facets.abort(name, new Error('nimbus-wrangler rebuilding'));
                    }
                    catch { }
                }
            }
    ...
    buildInnerEnv() {
        ...
        if (this.config?.worker_loaders?.length) {
            ...
            if (!ctxExports?.NimbusLoaderRPC) {
                ...
                const currentDepth = this.loaderEnv?.NIMBUS_INNER_LOADER_DEPTH_CURRENT
                    ? parseInt(String(this.loaderEnv.NIMBUS_INNER_LOADER_DEPTH_CURRENT), 10) || 0
                    : 0;
    ...
    outerWorkerBase) {
        ...
        try {
            ...
            return Response.json({ error: e?.message || String(e) }, { status: 500, headers: { 'Access-Control-Allow-Origin': '*' } });
        }
    }
    get stats() {
        return {
