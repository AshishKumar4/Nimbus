...
class BunReplAdapter {
    ...
    banner() {
        return (`Bun ${BUN_VERSION} emulation (Nimbus, over Cloudflare workerd)\r\n` +
 ...
 */
function bunReplStepFacetFn(args) {
    const g = globalThis;
    return (async function () {
        if (args.mode === 'init') {
            ...
            // a property mutation.
            const stdoutPush = (xs) => g.__nimbus_bun_stdout.push(xs.map((x) => typeof x === 'string' ? x : utilMod.inspect(x, { colors: false })).join(' ') + '\n');
            const stderrPush = (xs) => g.__nimbus_bun_stderr.push(xs.map((x) => typeof x === 'string' ? x : utilMod.inspect(x, { colors: false })).join(' ') + '\n');
            ...
            if (g.process && typeof g.process === 'object') {
                ...
                g.process.exit = function (code) {
                    const err = new Error('__nimbus_bun_exit__');
        ...
        // displayhook.
        if (!g.__nimbus_bun_history)
            g.__nimbus_bun_history = [];
        let result;
