 ...
 */
export async function fetchSqliteWasmBytes(env) {
    ...
    if (!ab) {
        ...
        try {
            if (!res.ok) {
                throw new Error(`sql.js wasm asset fetch failed: ${res.status} ${res.statusText} ` +
    ...
    if (digest !== SQLITE_WASM_SHA256) {
        throw new Error(`sql.js wasm integrity check failed: expected ${SQLITE_WASM_SHA256}, got ` +
