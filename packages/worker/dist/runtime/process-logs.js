...
export class ProcessLogStore {
        ...
        for (let i = state.chunks.length - 1; i >= 0; i--) {
            ...
            if (opts.lines) {
                ...
                for (let j = 0; j < c.data.length; j++)
                    if (c.data.charCodeAt(j) === 10)
                        lines++;
                if (lines >= opts.lines)
                    break;
            }
            if (opts.bytes && bytes >= opts.bytes)
                break;
        }
        return out;
    }
    ...
    flush() {
        ...
        for (const [pid, cutoff] of this._pruneQueue) {
            try {
                adapter.pruneBeforeSeq(pid, cutoff);
            }
            catch { /* fail-soft */ }
        }
        this._pruneQueue.clear();
        ...
        for (const [pid, state] of this.pids) {
            ...
            try {
                ...
                for (const r of rows) {
                    ...
                    if (r.seq > state.flushedHighSeq)
                        state.flushedHighSeq = r.seq;
                }
    ...
    hibStats() {
        ...
        for (const state of this.pids.values()) {
            if (state.dirtyChunks.length > 0 || state.dirtyExit)
                pendingDirtyPids++;
        }
        return {
    ...
    _evict(state, pid) {
        ...
        if (!droppedAny || pid === undefined || !this._persist)
            return;
        ...
        if (cutoff > existing)
            this._pruneQueue.set(pid, cutoff);
        if (state.dirtyChunks.length > 0) {
