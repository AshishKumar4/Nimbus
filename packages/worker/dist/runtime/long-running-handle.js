 ...
 */
export function makeLongRunningPortStub(handler) {
    return {
 ...
 */
export function parsePortFromArgv(argv) {
    let result = null;
 ...
 */
export function expandShellDefaults(token, env) {
    ...
    return token.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)(?::-([^}]*))?\}/g, (_m, name, def) => {
        ...
        if (def.indexOf('${') >= 0)
            return expandShellDefaults(def, env);
        return def;
    });
}
/**
 ...
 * fully resolved.
 */
export function expandArgvShellDefaults(argv, env) {
 ...
 */
export function pickDefaultPreviewPort(ports) {
    if (ports.length === 0)
        return null;
    ...
    for (const p of ports)
        if (p.registeredAt < oldest.registeredAt)
            oldest = p;
    return oldest.port;
}
