...
function __wasiEnqueue(op, run) {
    ...
    __wasiPersistQ.tail = __wasiPersistQ.tail.then(async () => {
        try {
            ...
            if (i >= 0)
                __wasiPersistQ.pending.splice(i, 1);
        }
    });
}
...
function __wasiEvictCleanContent() {
    if (!__wasiFS || !__wasiSup)
        return [];
    const evicted = [];
    for (const [path, bytes] of [...__wasiFS.files]) {
        if (__wasiDirty.has(path))
            continue;
        const orig = __wasiFS.origFiles.get(path);
        if (!orig || orig.length !== bytes.length)
            continue;
        let same = true;
 ...
 */
export async function __wasiRevalidateFS() {
    if (!__wasiSup || !__wasiFS)
        return [];
    ...
    if (typeof __wasiSup.fsRevision === 'function') {
        ...
        if (revision !== null && revision === __wasiFS.revision)
            return [];
 ...
 */
export async function __wasiDrainPersist() {
...
export function __wasiInitFS(opts) {
    ...
    for (const [path, t] of Object.entries(opts.times || {})) {
        ...
        if (!cur)
            continue; // path not yet in files/dirs; skip
    ...
    for (const po of (opts.preopens || [])) {
        ...
        if (!times.has(vfsPath))
            times.set(vfsPath, { mtime: nowNs, atime: nowNs, ctime: nowNs });
    }
}
...
function __wasiOpenTcpSocket(pathArg, fdflags, fdOutPtr, writeU32LE) {
    ...
    if (opened.errno !== __WASI_ESUCCESS)
        return opened.errno;
 ...
 */
export function __wasiReadFilesB64(paths) {
    const out = {};
...
function __wasiAllocateFd() { return nextFd++; }
...
function __wasiResolvePath(baseFd, pathStr) {
    ...
    if (!entry)
        return null;
    let baseVfs;
    if (entry.kind === 'preopen' || entry.kind === 'dir')
        baseVfs = entry.vfsPath;
    else
        return null;
...
function __wasiResolvePathFull(baseFd, pathStr, followFlag) {
    ...
    if (p === null)
        return { path: '', isSymlink: false, err: __WASI_EBADF };
    if (!followFlag) {
...
function __wasiInodeExists(path) {
    return (__wasiFS.files.has(path) ||
...
function __wasiCheckTraversal(baseFd, resolved) {
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if ((__wasiEffectiveMode(ancestor) & 1) === 0)
...
export function __wasiMakeImports(opts) {
    ...
    function readRange(vfsPath, offset, length) {
        ...
        return (async () => {
            ...
            if (size <= __wasiFS.residentFileCap) {
                ...
                if (whole === null)
                    throw new Error('ENOENT: ' + vfsPath);
                const bytes = whole;
    ...
    function underEnumeratedRoot(vfsPath) {
        for (const root of __wasiFS.enumeratedRoots) {
            if (root === '' || vfsPath === root || vfsPath.startsWith(root + '/'))
                return true;
        }
        return false;
    }
    function statLive(vfsPath) {
        if (!__wasiSup || __wasiNegative.has(vfsPath))
            return false;
        ...
        if (underEnumeratedRoot(vfsPath))
            return false;
        return (async () => {
            await __wasiDrainPersist();
            const st = await __wasiSup.stat(vfsPath);
            if (!st) {
                __wasiNegative.add(vfsPath);
                return false;
            }
    ...
    function ensureParentDirs(vfsPath) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
            ...
            __wasiFS.dirs.add(p);
            if (!__wasiFS.modes.has(p))
                __wasiFS.modes.set(p, 7);
    ...
    const imports = {
        ...
        args_sizes_get(argcPtr, sizePtr) {
            let total = 0;
            for (let i = 0; i < argv.length; i++)
                total += utf8enc.encode(argv[i]).length + 1;
        ...
        environ_sizes_get(envcPtr, sizePtr) {
            let total = 0;
            for (let i = 0; i < envArr.length; i++)
                total += utf8enc.encode(envArr[i]).length + 1;
        ...
        fd_read(fd, iovsPtr, iovsLen, nreadPtr) {
            ...
            if (entry.kind === 'socket')
                return __rawSockRecv(fd, iovsPtr, iovsLen, 0, nreadPtr, 0);
        ...
        fd_seek(fd, offsetArg, whence, newOffsetPtr) {
            ...
            if (entry && (entry.kind === 'socket' || entry.kind === 'listener'))
                return __WASI_ESPIPE;
            if (!entry || entry.kind !== 'file')
                return __WASI_EBADF;
        ...
        fd_fdstat_set_flags(fd, flags) {
            ...
            if (entry.kind === 'file' || entry.kind === 'socket' || entry.kind === 'listener')
                entry.fdflags = flags;
            return __WASI_ESUCCESS;
        },
        ...
        path_open(baseFd, dirflags, pathPtr, pathLen, oflags, rightsBase, _rightsInheriting, fdflags, fdOutPtr) {
            ...
            const fileExists = __wasiFS.files.has(resolved) || __wasiFS.sizes.has(resolved) || (__wasiFS.modes.has(resolved) &&
            ...
            let requiredMode = 0;
            if ((requested & __WASI_RIGHT_FD_READ) !== 0n)
                requiredMode |= 4;
            if ((requested & __WASI_RIGHT_FD_WRITE) !== 0n)
                requiredMode |= 2;
            if (requiredMode !== 0) {
        ...
        path_rename(srcFd, srcPathPtr, srcPathLen, dstFd, dstPathPtr, dstPathLen) {
            ...
            if (srcFile === undefined && !srcIsLazy && !srcIsDir && !srcIsSymlink)
                return __WASI_ENOENT;
            if (src === dst)
                return __WASI_ESUCCESS; // rename to itself is a no-op
        ...
        path_filestat_get(baseFd, lookupflags, pathPtr, pathLen, statPtr) {
            ...
            const classify = () => {
                ...
                if (__wasiFS.dirs.has(resolved))
                    return [__WASI_FT_DIRECTORY, 0n];
                if (__wasiFS.modes.has(resolved))
                    return [__WASI_FT_REGULAR_FILE, 0n];
                return null;
            };
            let kind = classify();
            if (!kind) {
        ...
        path_filestat_set_times(baseFd, lookupflags, pathPtr, pathLen, atimArg, mtimArg, fstflags) {
            ...
            if ((setAtim && setAtimNow) || (setMtim && setMtimNow))
                return __WASI_EINVAL;
            const now = __wasiNowNs();
        ...
        path_readlink(baseFd, pathPtr, pathLen, bufPtr, bufLen, bufUsedPtr) {
            ...
            const resolved = rp.path;
            if (!__wasiFS.symlinks.has(resolved))
                return __WASI_EINVAL; // not a symlink
        ...
        path_link(oldFd, oldFlags, oldPathPtr, oldPathLen, newFd, newPathPtr, newPathLen) {
            ...
            if (rpOld.err === __WASI_EBADF)
                return __WASI_EBADF;
            if (rpOld.err === __WASI_ELOOP)
                return __WASI_ELOOP;
            const src = rpOld.path;
            const dst = __wasiResolvePath(newFd, newPath);
            if (dst === null)
                return __WASI_EBADF;
            if (!__wasiFS.files.has(src))
                return __WASI_ENOENT;
        ...
        fd_readdir(fd, bufPtr, bufLen, cookieArg, bufusedPtr) {
            const entry = fdTable.get(fd);
            if (!entry || (entry.kind !== 'dir' && entry.kind !== 'preopen'))
                return __WASI_EBADF;
            ...
            for (let i = startCookie; i < entry.readdirEntries.length; i++) {
                ...
                for (let j = 21; j < 24; j++)
                    dv.setUint8(bufPtr + written + j, 0);
                written += 24;
        ...
        fd_allocate(fd, offsetArg, lenArg) {
            ...
            if (needed <= cur.length)
                return __WASI_ESUCCESS; // already big enough
        ...
        async poll_oneoff(inSubsPtr, outEventsPtr, nsubs, retNeventsPtr) {
            ...
            const promises = subs.map((s) => {
                ...
                if (entry.kind === 'listener') {
                    ...
                    if (kernel.pending(entry.port) > 0)
        ...
        async sock_send(fd, siDataPtr, siDataLen, _siFlags, retDataLenPtr) {
            ...
            if (entry.closed || entry.halfClosedWr)
                return __WASI_EPIPE;
            ...
            try {
                ...
                if (/unreach|EHOSTUNREACH/i.test(msg))
    ...
    if (opts.parking === 'none') {
        ...
        if (opts.threads) {
            throw new Error('wasi: a threads build cannot run with parking:none; its threads block on a '
        ...
        for (const name of parkable) {
            ...
            imports[name] = function noPark(...args) {
                const r = fn.apply(this, args);
                if (r && typeof r.then === 'function') {
                    throw new Error('wasi: ' + name + ' parked in a non-suspending instance; a guest that '
        ...
        if (opts.threads)
            imports.sched_yield = new WebAssembly.Suspending(imports.sched_yield);
    }
    return {
        wasiImport: imports,
        getStdout: () => stdoutBuf,
...
export function __wasiRunStart(instance, ctx) {
    try {
...
export async function __wasiRunStartAsync(instance, ctx) {
    try {
        const start = instance.exports._start;
        if (typeof start !== 'function') {
            return { exitCode: 1, error: '_start is not a function (got ' + typeof start + ')' };
