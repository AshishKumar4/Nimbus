...
class NodeReplAdapter {
    ...
    banner() {
        return (`Node.js ${NODE_VERSION} compatibility layer (Nimbus, over Cloudflare workerd)\r\n` +
 ...
 */
function nodeReplStepFacetFn(args) {
    const g = globalThis;
    return (async function () {
        if (args.mode === 'init') {
            ...
            const stdoutPush = (xs) => g.__nimbus_node_stdout.push(xs.map((x) => typeof x === 'string' ? x : utilMod.inspect(x, { colors: false })).join(' ') + '\n');
            const stderrPush = (xs) => g.__nimbus_node_stderr.push(xs.map((x) => typeof x === 'string' ? x : utilMod.inspect(x, { colors: false })).join(' ') + '\n');
            g.__nimbus_node_orig_console = g.console;
            g.console = {
                log: (...xs) => stdoutPush(xs),
                error: (...xs) => stderrPush(xs),
                warn: (...xs) => stderrPush(xs),
                info: (...xs) => stdoutPush(xs),
                debug: (...xs) => stdoutPush(xs),
            };
            if (g.process && typeof g.process === 'object') {
                g.__nimbus_node_orig_exit = g.process.exit;
                g.process.exit = function (code) {
                    const err = new Error('__nimbus_node_exit__');
        ...
        // displayhook.
        if (!g.__nimbus_node_history)
            g.__nimbus_node_history = [];
        let result;
        let evaluatedAs = 'stmt';
        try {
 ...
 */
export async function runNodeRepl(deps) {
