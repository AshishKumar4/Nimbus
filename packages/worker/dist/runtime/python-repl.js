...
class PythonReplAdapter {
    pool = null; // NimbusLoaderPool (typed loosely to avoid cyclic import)
    initDone = false;
    deps;
    ps1 = '>>> ';
    ps2 = '... ';
    ...
    banner() {
        return ('Python 3.13.2 (Pyodide 0.29.4) on Nimbus\r\n' +
    ...
    async ensurePool() {
        ...
        this.__wasmBytesAB = toAB(wasmBytes);
    }
    async submitFacetFn(args) {
        const wasmModules = { 'pyodide.asm.wasm': this.__wasmBytesAB };
    ...
    return (async function () {
        ...
        if (args.mode === 'init') {
            ...
            try {
                ...
                g.__nimbus_repl_stdout_cb = function (s) { g.__nimbusPyStdout.push(s); };
                g.__nimbus_repl_stderr_cb = function (s) { g.__nimbusPyStderr.push(s); };
                ...
                return {
                    stdout: '',
                    stderr: '',
                    error: 'PyodideConsole init failed: ' + (e?.message || e),
                };
            }
        ...
        let stepRes;
        try {
            stepRes = pyodide.runPython('__nimbus_repl_step(__import__("js").__nimbus_repl_src)');
        }
        catch (e) {
        ...
        try {
            ...
            finishRes = await pyodide.runPythonAsync('await __nimbus_repl_finish(__import__("js").__nimbus_repl_fut)');
        }
        catch (e) {
