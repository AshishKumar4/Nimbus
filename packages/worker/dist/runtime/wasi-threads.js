...
function readVarU32(bytes, at) {
    ...
    for (;;) {
        if (i >= bytes.length)
            throw new Error('wasm: truncated LEB128');
        const b = bytes[i++];
        result |= (b & 0x7f) << shift;
        if ((b & 0x80) === 0)
            return [result >>> 0, i];
        shift += 7;
        if (shift > 35)
            throw new Error('wasm: LEB128 too long');
    }
}
/**
 ...
 */
export function inspectWasmThreads(bytes) {
    const info = { spawns: false, futex: false, memory: null, threadStart: false };
    if (bytes.length < 8)
        return info;
        ...
        if (id === 2) {
            ...
            for (let n = 0; n < count; n++) {
                ...
                if (kind === 0x00) {
                    ...
                    if (mod === WASI_THREADS_NAMESPACE && name === 'thread-spawn')
                        info.spawns = true;
                    if (mod === NIMBUS_THREADS_NAMESPACE && name === 'futex_wait')
                        info.futex = true;
                }
                else if (kind === 0x01) {
                    p++; // reftype
            ...
            for (let n = 0; n < count; n++) {
                ...
                const kind = bytes[p++];
                [, p] = readVarU32(bytes, p);
                if (kind === 0x00 && name === WASI_THREAD_START_EXPORT)
                    info.threadStart = true;
            }
        }
        i = end;
    }
 ...
 */
const THREADS_LINK_FLAGS = '--target=wasm32-wasip1-threads -pthread '
 ...
 */
export function wasiThreadsLoadError(info) {
    if (!info.spawns)
        return null;
    if (!info.memory) {
