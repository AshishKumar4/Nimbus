 ...
 */
export function scanNamedImports(vfs, projDir) {
    const result = new Map();
    const scanExts = new Set(['.ts', '.tsx', '.jsx', '.js', '.mjs']);
    const add = (pkgName, name) => {
        let set = result.get(pkgName);
        if (!set) {
            set = new Set();
            result.set(pkgName, set);
        }
        set.add(name);
    };
    ...
    const walk = (dir, depth) => {
        ...
        for (const entry of entries) {
            ...
            while ((m = namedImportRe.exec(code)) !== null) {
                ...
                if (specRaw.startsWith('.') || specRaw.startsWith('/') || specRaw.startsWith('node:'))
                    continue;
...
export function buildSyntheticEntry(vfs, nmDir, pkgName, names) {
    if (names.size === 0)
        return null;
    ...
    const indexCode = (() => {
        try {
            return vfs.readFileString(indexPath);
        }
        catch {
            return null;
        }
    ...
    while ((m = reExport.exec(indexCode)) !== null) {
        ...
        const path = m[2];
        if (!path.startsWith('.'))
            continue; // only relative re-exports
        for (const partRaw of body.split(',')) {
            const part = partRaw.trim();
            if (!part)
                continue;
    ...
    while ((s = reExportStar.exec(indexCode)) !== null && starSourcesWalked < STAR_SOURCE_CAP) {
        ...
        let starCode;
        try {
            starCode = vfs.readFileString(fullPath);
        }
        catch {
            starCode = null;
        }
    ...
    for (const filePath of sortedFiles) {
        ...
        const memberLines = items.map(i => {
            if (i.isDefault)
                return `default as ${i.exportedName}`;
            if (i.sourceName === i.exportedName)
                return i.exportedName;
 ...
 */
export function buildScopedSliceForSynthetic(vfs, nmDir, pkgName, referencedFiles, transitiveCap = 800) {
    const entries = [];
    let totalBytes = 0;
    ...
    while (queue.length > 0 && visited.size < transitiveCap) {
        ...
        if (!vfs.exists(filePath) || vfs.isDirectory(filePath))
            continue;
        let bytes;
        try {
            bytes = vfs.readFile(filePath);
        }
        catch {
            continue;
        }
        ...
        if (slash > 0)
            addDir(filePath.substring(0, slash));
 ...
 */
function findPackageEsmEntry(vfs, nmDir, pkgName) {
    const pkgRoot = nmDir + '/' + pkgName;
    const pkgJsonPath = pkgRoot + '/package.json';
 ...
 */
export function syntheticEntryPath(projDir, pkgName) {
