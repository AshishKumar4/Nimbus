...
function humanSize(bytes) {
    ...
    if (amount >= base) {
        ...
        if (amount < 10) {
            ...
            if (amount < 10)
                return `${amount}.${tenths}${HUMAN_UNITS[exponent - 1]}`;
        }
    }
    if (tenths + rounding > 0) {
        amount++;
        if (amount === base && exponent < HUMAN_UNITS.length)
            return `1.0${HUMAN_UNITS[exponent]}`;
    }
...
`;
function parseDf(args) {
    const options = { all: false, human: false, printType: false, operands: [] };
...
function renderTable(rows, minimums, aligns) {
...
function dfRow(entry, usage, options) {
    const amount = (bytes) => options.human ? humanSize(bytes) : String(Math.ceil(bytes / 1024));
    ...
    if (usage === null) {
        ...
        row.push(amount(usage.size), amount(usage.used), amount(usage.available), total === 0 ? '-' : `${Math.ceil((usage.used * 100) / total)}%`);
    }
    row.push(entry.mountPoint);
    return row;
}
function createDfCommand(filesystem) {
    return async (ctx) => {
        const parsed = parseDf(ctx.args);
        if (parsed === 'help') {
            await ctx.stdout.write(DF_USAGE);
            return 0;
        }
        if ('error' in parsed) {
        ...
        selected.forEach(({ entry, explicit }, index) => {
            ...
            if (usage === null && !explicit && !parsed.all)
                return;
...
`;
function createMountCommand(filesystem) {
    return async (ctx) => {
        let types = null;
        const operands = [];
        for (let i = 0; i < ctx.args.length; i++) {
            ...
            if (arg === '-h' || arg === '--help') {
                await ctx.stdout.write(MOUNT_USAGE);
                return 0;
            }
            if (arg === '-l' || arg === '--show-labels')
                continue;
...
export function registerMountCommands(registry, filesystem) {
