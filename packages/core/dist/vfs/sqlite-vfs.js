...
export class SqliteVFS {
    ...
    initSchema() {
        ...
        next INTEGER NOT NULL
            ...
            // (repairIno).
            if (inoColumnAdded || allocatorSeeded)
            ...
            const kindTriggers = [...this.sql.exec(`SELECT COUNT(*) AS n FROM sqlite_master
         WHERE type = 'trigger' AND tbl_name = 'inodes'
           AND name IN ('trg_inodes_kind_insert', 'trg_inodes_kind_update')`)][0];
    ...
    nextIno() {
        ...
        if (!row)
            throw new Error('[sqlite-vfs] inode allocator row missing');
    ...
    openDescription(path, cred, rights) {
        ...
        const read = (offset, length) => {
            ...
            for (let i = Math.floor(start / CHUNK_SIZE); i * CHUNK_SIZE < end; i++) {
                ...
                if (to > from)
                    bytes.set(chunk.subarray(from - i * CHUNK_SIZE, to - i * CHUNK_SIZE), from - start);
            }
            return bytes;
        };
        const detachedResize = (size, offset = 0, bytes = new Uint8Array(0)) => {
            const node = current();
            ...
            this.transactionSync(() => {
                for (let i = first; i < count; i++) {
                    ...
                    if (old)
                        chunk.set(old.subarray(0, Math.min(old.byteLength, chunkLen)));
                    if (to > from)
                        chunk.set(bytes.subarray(from - offset, to - offset), from - start);
            ...
            node.size = size;
            node.chunkCount = count;
            node.mtime = node.ctime = this.now();
            for (const other of this.openNodes)
                if (other.path === null && this.contentIdForInode(other.inode) === contentId)
                    other.inode = node;
        };
        return {
            ...
            write: (offset, bytes) => {
                ...
                if (opened.path !== null)
                    ...
                    detachedResize(Math.max(node.size, start + bytes.length), start, bytes);
                return bytes.length;
            },
            truncate: size => {
                const node = current();
    ...
    _mkdirSingle(path, requestedMode, cred) {
        ...
        this.transactionSync(() => {
            ...
            this.sql.exec("INSERT OR REPLACE INTO inodes (path, parent_path, kind, size, atime, mtime, ctime, mode, uid, gid, chunk_count, content_id, ino) VALUES (?, ?, 1, 0, ?, ?, ?, ?, ?, ?, 0, NULL, ?)", path, pp, now, now, now, mode, cred.uid, cred.gid, ino);
        });
        const inode = {
            path,
            parentPath: pp,
    ...
    chmod(path, mode, cred) {
        ...
        const ctime = this.now();
        const entry = inode.isDir && full !== inode.mode ? accessChanged(inode.path) : NO_STRUCTURAL_CHANGES;
        this.sql.exec("UPDATE inodes SET mode = ?, ctime = ? WHERE path = ?", full, ctime, inode.path);
        inode.mode = full;
        inode.ctime = ctime;
        this.bumpRevision([inode.path], entry);
        this.emitMutation('change', inode.path);
    }
    chown(path, uid, gid, cred, followLeaf) {
        ...
        const ctime = this.now();
        const entry = inode.isDir && (nextUid !== inode.uid || nextGid !== inode.gid || nextMode !== inode.mode)
            ? accessChanged(inode.path)
            : NO_STRUCTURAL_CHANGES;
        this.sql.exec('UPDATE inodes SET uid = ?, gid = ?, mode = ?, ctime = ? WHERE path = ?', nextUid, nextGid, nextMode, ctime, inode.path);
        inode.uid = nextUid;
        inode.gid = nextGid;
        inode.mode = nextMode;
        inode.ctime = ctime;
    ...
    list(after, limit, cred) {
        ...
        for (;;) {
            const rows = [...this.sql.exec(`SELECT ${INODE_SELECT_COLUMNS} FROM inodes WHERE path > ? ORDER BY path LIMIT ?`, cursor, limit + 1)];
            for (const row of rows) {
    ...
    *subtreeDescending(root, rootInode, directoriesOnly) {
        ...
        for (;;) {
            const rows = [...(below === null
                    ? this.sql.exec(`SELECT ${INODE_SELECT_COLUMNS} FROM inodes WHERE path > ?${kind} ORDER BY path DESC LIMIT ?`, range.lower, SUBTREE_PAGE_ROWS)
                    : this.sql.exec(`SELECT ${INODE_SELECT_COLUMNS} FROM inodes WHERE path > ? AND path < ?${kind} ORDER BY path DESC LIMIT ?`, range.lower, below, SUBTREE_PAGE_ROWS))];
            for (const row of rows)
    ...
    executeTransactionPlan(plan, execution, onCommit, priors) {
        ...
        const prior = priors ?? plan.inodes.map((inode) => (inode.ino === undefined ? this.inodes.get(inode.path) : undefined));
        this.executeMeasuredTransaction(plan, execution, () => {
            ...
            for (let i = 0; i < plan.inodes.length; i += INODE_ROWS_PER_SQL_EXEC) {
                ...
                for (let k = 0; k < batch.length; k++) {
                    ...
                    values.push(inode.path, inode.parentPath, inodeKindCode(inode.kind), inode.size, atime, inode.mtime, inode.ctime ?? committedAt, inode.mode, inode.uid, inode.gid, inode.chunkCount, inode.contentId, ino);
                }
                this.sql.exec(`INSERT OR REPLACE INTO inodes (path, parent_path, kind, size, atime, mtime, ctime, mode, uid, gid, chunk_count, content_id, ino) VALUES ${placeholders}`, ...values);
            }
    ...
    _writeBatchOnce(prepared, execution, publishedChunkCount, onCommit) {
        ...
        // its name, and a reader holding anything under one must let it go.
        const removed = deletedInodes.filter((inode) => inode.isDir);
        for (let index = 0; index < plan.inodes.length; index++) {
            const prior = priors[index];
            if (prior?.isDir && !plan.inodes[index].isDir && !deleted.has(prior.path))
                removed.push(prior);
        }
        if (inodeCount > 0 || chunkCount > 0 || plan.deletedPaths.length > 0) {
            ...
            this.bumpRevision(Array.from(touched), removedDirectories(removed));
        }
        ...
        for (const root of roots) {
            ...
            const rows = [
                ...
                ...(range.upper === null
                    ...
                    : this.sql.exec(`SELECT ${INODE_SELECT_COLUMNS} FROM inodes WHERE path > ? AND path < ?`, range.lower, range.upper)),
            ];
            for (const row of rows) {
                const path = String(row.path);
                if (visited.has(path))
