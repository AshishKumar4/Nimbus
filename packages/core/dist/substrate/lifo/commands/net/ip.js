 ...
 */
export function createIPCommand(kernel) {
    return async (ctx) => {
        const args = ctx.args;
        if (args.length === 0) {
...
// ═══════════════════════════════════════════════════════════════
async function handleLink(ctx, command, args, networkStack) {
    switch (command) {
        case 'show':
        case 'list':
...
async function linkShow(ctx, args, networkStack) {
    let targetInterface;
    // Parse "dev NAME"
...
async function linkAdd(ctx, args, networkStack) {
    if (args.length < 3) {
...
async function linkDelete(ctx, args, networkStack) {
    if (args.length < 1) {
...
async function linkSet(ctx, args, networkStack) {
    if (args.length < 2) {
...
// ═══════════════════════════════════════════════════════════════
async function handleAddr(ctx, command, _args, _networkStack) {
    if (command === 'help') {
...
// ═══════════════════════════════════════════════════════════════
async function handleRoute(ctx, command, _args, _networkStack) {
    if (command === 'help') {
...
// ═══════════════════════════════════════════════════════════════
async function handleNetns(ctx, command, args, networkStack) {
    switch (command) {
        case 'list':
        case 'show':
...
async function netnsShow(ctx, _args, networkStack) {
    const namespaces = networkStack.getAllNamespaces();
    for (const ns of namespaces) {
...
async function netnsAdd(ctx, args, networkStack) {
    if (args.length < 1) {
...
async function netnsDelete(ctx, args, networkStack) {
    if (args.length < 1) {
...
// ═══════════════════════════════════════════════════════════════
async function handleBridge(ctx, command, args, networkStack) {
    switch (command) {
        case 'fdb':
...
async function bridgeFdb(ctx, args, networkStack) {
...
async function bridgeLink(ctx, args, networkStack) {
    const command = args[0] || 'show';
    if (command !== 'show') {
