...
export class Bridge {
    ...
    ports = new Map(); // interface name -> interface
    // MAC address learning table (like a real switch)
    macTable = new Map(); // MAC -> interface name
    // Forwarding database aging time (seconds)
    ageingTime = 300;
    // Last seen timestamps for MAC addresses
    lastSeen = new Map(); // MAC -> timestamp
    constructor(name, _networkStack, namespace = 'default') {
        this.name = name;
     ...
     */
    async forward(packet, ingressPort) {
        if (this.state === 'down') {
            return;
        }
     ...
     */
    async forwardToPort(packet, portName) {
     ...
     */
    async flood(packet, ingressPort) {
     ...
     */
    getSourceMAC(_packet, ingressPort) {
