...
function executeCjsCommand(source, entryPath, vfs, ctx, lifo) {
    ...
    function executeModule(modSource, modFilename) {
        ...
        function modRequire(n) {
            ...
            if (n.startsWith('./') || n.startsWith('../')) {
                ...
                const idx = join(abs, 'index.js');
                if (vfs.exists(idx)) {
                    if (moduleCache.has(idx))
                        return moduleCache.get(idx);
        ...
        fn(modExports, modRequire, modModule, modFilename, modDir, modConsole, modProcess, Buffer, globalThis.setTimeout, globalThis.setInterval, globalThis.clearTimeout, globalThis.clearInterval, {});
    ...
    try {
        const fn = new Function('return ' + wrapped)();
        fn(exports, lifoRequire, module, entryPath, entryDir, nodeConsole, cjsProcess, Buffer, globalThis.setTimeout, globalThis.setInterval, globalThis.clearTimeout, globalThis.clearInterval, {});
 ...
 */
export function readLifoManifest(vfs, pkgDir) {
    const pkgJsonPath = join(pkgDir, 'package.json');
    try {
        const pkg = JSON.parse(vfs.readFileString(pkgJsonPath));
