 ...
 */
export interface VfsMutationReceipt {
    before: number;
    after: number;
}
export interface RuntimeFsBridge {
    ...
    writeBatch(payload: import('@nimbus-sh/platform/w7-frame.js').BatchWritePayload): Awaitable<{
        inodes: number;
        chunks: number;
    }>;
