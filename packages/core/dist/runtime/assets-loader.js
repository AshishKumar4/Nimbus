 ...
 * fetch.
 */
const __assetTextCache = new Map();
const __assetBytesCache = new Map();
/**
 ...
 */
export function loadAssetBytes(assets, path) {
    if (!assets || typeof assets.fetch !== 'function') {
        return Promise.reject(new NimbusAssetLoadError(`ASSETS binding missing — cannot load ${path}. Did you forget the assets.binding=ASSETS in wrangler.jsonc?`, 'E_ASSETS_BINDING_MISSING', path));
    }
    ...
    const promise = (async () => {
        ...
        try {
            if (r.status === 404) {
                throw new NimbusAssetLoadError(`Asset not found: ${path}`, 'E_ASSET_NOT_FOUND', path, 404);
            }
            if (!r.ok) {
                throw new NimbusAssetLoadError(`Asset fetch failed: ${path} → ${r.status}`, 'E_ASSET_FETCH_FAILED', path, r.status);
            }
 ...
 * {@link loadAssetBytes}.
 */
export function loadAssetText(assets, path) {
    if (!assets || typeof assets.fetch !== 'function') {
        return Promise.reject(new NimbusAssetLoadError(`ASSETS binding missing — cannot load ${path}. Did you forget the assets.binding=ASSETS in wrangler.jsonc?`, 'E_ASSETS_BINDING_MISSING', path));
    }
 ...
 */
export function _resetAssetsCacheForTests() {
