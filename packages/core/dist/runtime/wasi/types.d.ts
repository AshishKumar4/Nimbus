...
export interface WasiImports {
    ...
    path_open(baseFd: number, dirflags: number, pathPtr: number, pathLen: number, oflags: number, rightsBase: bigint | number, rightsInheriting: bigint | number, fdflags: number, fdOutPtr: number): SyscallResult;
    ...
    path_rename(srcFd: number, srcPathPtr: number, srcPathLen: number, dstFd: number, dstPathPtr: number, dstPathLen: number): SyscallResult;
    path_filestat_get(baseFd: number, lookupflags: number, pathPtr: number, pathLen: number, statPtr: number): SyscallResult;
    path_filestat_set_times(baseFd: number, lookupflags: number, pathPtr: number, pathLen: number, atim: bigint | number, mtim: bigint | number, fstflags: number): SyscallResult;
    path_readlink(baseFd: number, pathPtr: number, pathLen: number, bufPtr: number, bufLen: number, bufUsedPtr: number): SyscallResult;
    path_symlink(oldPathPtr: number, oldPathLen: number, newFd: number, newPathPtr: number, newPathLen: number): SyscallResult;
    path_link(oldFd: number, oldFlags: number, oldPathPtr: number, oldPathLen: number, newFd: number, newPathPtr: number, newPathLen: number): SyscallResult;
 ...
 */
export type ParkableImport = Extract<keyof WasiImports, 'sock_send' | 'sock_recv' | 'sock_shutdown' | 'sock_accept' | 'poll_oneoff' | 'fd_read' | 'fd_write' | 'fd_pread' | 'fd_pwrite' | 'path_filestat_get' | 'path_open' | 'fd_close' | 'fd_renumber' | 'fd_seek' | 'fd_tell' | 'fd_filestat_get' | 'fd_fdstat_set_flags' | 'fd_fdstat_set_rights' | 'fd_filestat_set_size' | 'fd_sync' | 'fd_datasync' | 'fd_allocate' | 'fd_advise' | 'fd_readdir' | 'path_create_directory' | 'path_remove_directory' | 'path_unlink_file' | 'path_rename' | 'path_symlink' | 'path_readlink' | 'path_link' | 'fd_filestat_set_times' | 'path_filestat_set_times'>;
/**
...
declare global {
    namespace WebAssembly {
        ...
        function promising<T extends (...args: never[]) => unknown>(fn: T): (...args: Parameters<T>) => Promise<Awaited<ReturnType<T>>>;
    }
