 ...
 */
export function shouldProcessForRouter(vfsPath) {
    if (!vfsPath)
        return false;
 ...
 */
export function injectRouterBasename(src, basePath) {
    // Fast short-circuits
    if (!src || src.length === 0)
        return src;
    ...
    if (!mentionsFactory && !mentionsComponent)
        return src;
    ...
    out = out.replace(/\bcreateBrowserRouter\s*\(\s*([\s\S]*?)\s*,\s*\{([\s\S]*?)\}\s*\)/g, (match, routes, opts) => {
        // Balance check: if `routes` contains unmatched parens, our regex ate too much.
        if (!isBalanced(routes))
            return match;
        if (/\bbasename\s*:/.test(opts))
            return match; // user set basename
    ...
    out = out.replace(/<BrowserRouter((?:\s+[^>\/\s][^>]*?)?)(\s*\/?\s*>)/g, (match, attrs, end) => {
        if (attrs && /\bbasename\s*=/.test(attrs))
            return match;
 ...
 */
function scanAndReplaceCreateBrowserRouterSingleArg(src, basenameLiteral) {
