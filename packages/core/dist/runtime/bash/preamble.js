...
function newSession(args) {
    ...
    for (const [path, b64] of Object.entries(snap.files || {}))
        fs.files.set(norm(path), { bytes: b64ToBytes(b64) });
    ...
    for (const [path, m] of Object.entries(snap.modes || {}))
        fs.modes.set(norm(path), Number(m) & 7);
    for (const seed of ['etc', 'dev', 'bin', 'usr', 'usr/bin']) {
        fs.dirs.add(seed);
        if (!fs.modes.has(seed))
            fs.modes.set(seed, 5);
    }
    ...
    if (args.stdinTty && home && !fs.files.has(home + '/.bashrc')) {
        ...
        if (!fs.modes.has(home + '/.bashrc'))
            fs.modes.set(home + '/.bashrc', 6);
    }
...
function followPath(s, path) {
    ...
    for (let hops = 0; hops < 32; hops++) {
        if (!s.fs.symlinks.size)
            return { path: cur, errno: 0 };
...
function checkTraversal(s, path) {
    ...
    for (let i = 0; i < parts.length - 1; i++) {
        ...
        if (!isDir(s, anc))
            return inodeExists(s, anc) ? E.NOTDIR : E.NOENT;
        if ((effMode(s, anc) & 1) === 0)
            return E.ACCES;
    }
    return 0;
}
function checkParentWritable(s, path) {
    const parent = parentOf(path);
    if (parent && (effMode(s, parent) & 3) !== 3)
        return E.ACCES;
    return 0;
}
function recordDirAdded(s, path) {
    ...
    if (!s.fs.modes.has(path))
        s.fs.modes.set(path, 7);
    touchPath(s, path);
}
function recordDirRemoved(s, path) {
    ...
    if (s.fs.dirsCreated.has(path))
        s.fs.dirsCreated.delete(path);
    else
        s.fs.dirsDeleted.add(path);
}
...
function markWritten(s, path) {
    ...
    if (!s.fs.modes.has(path))
        s.fs.modes.set(path, 6);
    touchPath(s, path);
    const parts = path.split('/');
    for (let i = 1; i < parts.length; i++) {
        const dir = parts.slice(0, i).join('/');
        if (!s.fs.dirs.has(dir))
            recordDirAdded(s, dir);
    }
}
...
function makeResolve(s, pathBase, absRoots) {
    return (raw) => {
        ...
        if (!pathBase)
            return p;
        if (isUnderAbsRoot(p, absRoots))
            return p;
        const joined = norm(pathBase + '/' + p);
        if (inodeExists(s, joined))
            return joined;
        if (inodeExists(s, p))
            return p;
        if (isDir(s, parentOf(joined)))
            return joined;
    ...
    return {
        ...
        path_open: (dirfd, dirflags, pathPtr, pathLen, oflags, rightsBase, _ri, fdflags, retPtr) => {
            ...
            if (path === 'dev/null' || path === 'dev/tty') {
                ...
                proc.fds.set(fd, { kind: path === 'dev/tty' ? 'tty' : 'null' });
            ...
            if (isDir(s, path)) {
                if ((effMode(s, path) & 1) === 0)
                    return E.ACCES;
                const fd = lowestFd(proc);
                proc.fds.set(fd, { kind: 'dir', path });
                dv.setUint32(retPtr, fd, true);
                return 0;
            }
            if (wantDir)
                return fileExists(s, path) ? E.NOTDIR : E.NOENT;
            ...
            if ((rights & 64n) !== 0n)
                need |= 2;
            if ((oflags & 8) !== 0)
                need |= 2; // O_TRUNC implies write
            let entry = fileLookup(s, path);
            const exists = entry !== null || fileExists(s, path);
            if (exists && (oflags & 4))
                return E.EXIST; // O_EXCL
            if (!exists) {
                if (!(oflags & 1))
                    return E.NOENT; // no O_CREAT
                ...
                if (need && (effMode(s, path) & need) !== need)
                    return E.ACCES;
                if (!entry) {
        ...
        path_filestat_set_times: (dirfd, _flags, pathPtr, pathLen, _atim, mtim, fstflags) => {
            ...
            if (!inodeExists(s, path) && !s.fs.symlinks.has(path))
                return E.NOENT;
            if (fstflags & 8)
                s.fs.times.set(path, realtimeNs());
            else if (fstflags & 4)
                s.fs.times.set(path, typeof mtim === 'bigint' ? mtim : BigInt(mtim >>> 0));
            return 0;
        },
        path_unlink_file: (dirfd, pathPtr, pathLen) => {
            ...
            if (isDir(s, path))
                return E.ISDIR;
            if (!fileExists(s, path) && !s.fs.symlinks.has(path))
                return E.NOENT;
            const denied = checkParentWritable(s, path);
            if (denied)
                return denied;
        ...
        path_rename: (fd1, oldPtr, oldLen, fd2, newPtr, newLen) => {
            ...
            if (!fromIsDir && !f && !fileExists(s, from))
                return E.NOENT;
            if (from === to)
                return 0;
            let denied = checkParentWritable(s, from);
            if (!denied)
                denied = checkParentWritable(s, to);
            if (denied)
                return denied;
            const moveMode = (a, b) => {
        ...
        path_create_directory: (dirfd, pathPtr, pathLen) => {
            ...
            if (trav)
                return trav;
            if (isDir(s, path) || fileExists(s, path))
                return E.EXIST;
            const denied = checkParentWritable(s, path);
            if (denied)
                return denied;
        ...
        path_readlink: (dirfd, pathPtr, pathLen, bufPtr, bufLen, bufUsedPtr) => {
            ...
            if (target === undefined)
                return inodeExists(s, r.path) ? E.INVAL : E.NOENT;
        ...
        path_symlink: (oldPtr, oldLen, newFd, newPtr, newLen) => {
            ...
            if (trav)
                return trav;
            if (inodeExists(s, link) || s.fs.symlinks.has(link))
                return E.EXIST;
        ...
        path_link: (fd1, _lookupFlags, oldPtr, oldLen, fd2, newPtr, newLen) => {
            ...
            const f = fileLookup(s, from);
            if (!f)
                return fileExists(s, from) ? E.ACCES : E.NOENT;
            if (fileExists(s, to) || isDir(s, to))
                return E.EXIST;
        ...
        path_remove_directory: (dirfd, pathPtr, pathLen) => {
            ...
            if (trav)
                return trav;
            if (path === '' || path === 'etc' || path === 'dev' || path === 'bin' || path === 'usr' || path === 'usr/bin')
                return E.ACCES;
            if (!isDir(s, path))
                return fileExists(s, path) ? E.NOTDIR : E.NOENT;
            const denied = checkParentWritable(s, path);
        ...
        fd_fdstat_get: (fd, st) => {
            ...
            if (e && e.kind === 'pipe')
                ft = 0;
            else if (e && (e.kind === 'stdin' || e.kind === 'stdout' || e.kind === 'stderr'))
                ft = s.stdinTty || fd !== 0 ? 2 : 0;
            else if (e && e.kind === 'tty')
                ft = 2;
            else if (e && (e.kind === 'dir' || e.kind === 'preopen'))
                ft = 3;
            else if (!e)
                return E.BADF;
            dv.setUint8(st, ft);
...
function writeThroughFd(s, proc, fd, bytes) {
    ...
    if (e && e.kind === 'file') {
        ...
        if (!s.fs.files.has(e.path))
            s.fs.files.set(e.path, f);
...
function doExec(s, proc) {
    ...
    const w = {
        ...
        args_get: (ptrs, buf) => { const dv = DV(); let p = buf; for (const a of tv) {
            dv.setUint32(ptrs, p, true);
            ptrs += 4;
            p += wstr(p, a);
            U8()[p++] = 0;
        ...
        environ_get: (ptrs, buf) => { const dv = DV(); let p = buf; for (const v of env) {
            dv.setUint32(ptrs, p, true);
            ptrs += 4;
            p += wstr(p, v);
            U8()[p++] = 0;
    ...
    const nimbus_proc = {
        chmod: (pathPtr, pathLen, mode) => {
            ...
            if (trav)
                return trav;
            if (!inodeExists(s, p) && !(p !== '' && isDir(s, p)))
                return E.NOENT;
    ...
    if (mem2 > s.stats.memPeak)
        s.stats.memPeak = mem2;
    try {
...
function composeFsDiff(s) {
    ...
    for (const path of s.fs.written) {
        if (path.startsWith('etc/nimbus.bashrc'))
            continue;
        const f = s.fs.files.get(path);
        if (f)
            filesWritten[path] = bytesToB64(f.bytes);
    }
    return {
        filesWritten,
 ...
 */
globalThis.__bashStep = function __bashStep(raw) {
