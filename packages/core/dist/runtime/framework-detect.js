...
function readViteConfig(input) {
    if (!input.fileContents)
        return null;
    for (const c of VITE_CONFIG_FILES) {
        if (input.fileContents[c])
            return { name: c, contents: input.fileContents[c] };
    }
    return null;
}
/**
    ...
    if (wranglerCfg) {
        ...
        if (underlying.framework !== 'unknown' && underlying.framework !== 'wrangler') {
            return {
                ...
                confidence: 0.85,
                reason: `wrangler config (${wranglerCfg}) AND ${underlying.reason} — routing through ` +
...
function detectUnderlyingFramework(input) {
    const { pkg, files } = input;
    // Step 1: Next.js
    if (hasDep(pkg, 'next')) {
        return {
 ...
 */
export function describeDetect(result) {
    return (`framework=${result.framework} dev=${result.devCommand} ` +
