...
export function makeNimbusVerbHandler(deps: NimbusVerbDeps): (ctx: CommandContext) => Promise<number> {
  return async function nimbus(ctx: CommandContext): Promise<number> {
    const argv = ctx.args || [];
    const verb = argv[0];
...
async function warmInstalledRuntime(
  seeded: { name: string; version: string; root: string },
  ctx: NimbusShellCtx,
  deps: NimbusVerbDeps,
): Promise<void> {
...
export async function runNimbusInstall(
  args: string[],
  ctx: NimbusShellCtx,
  deps: NimbusVerbDeps,
): Promise<number> {
  ...
  if (listOnly) {
    ...
    for (const runtime of installed) {
      ctx.stdout.write(
        `  ${runtime.name}@${runtime.version}  abi=${runtime.abi}  `
  ...
  if (availOnly) {
    ...
    for (const runtime of sorted) {
      ...
      ctx.stdout.write(
        `  ${runtime.name}  abi=${runtime.abi}  default=${runtime.defaultVersion}  versions=[${versions.join(', ')}]\n`,
      );
...
async function runNimbusUninstall(
  args: string[],
  ctx: NimbusShellCtx,
  deps: NimbusVerbDeps,
): Promise<number> {
...
async function runExpose(args: string[], ctx: NimbusShellCtx, apps: NimbusAppVerbs): Promise<number> {
...
async function runApp(args: string[], ctx: NimbusShellCtx, apps: NimbusAppVerbs): Promise<number> {
