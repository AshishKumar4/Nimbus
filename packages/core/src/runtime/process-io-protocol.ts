...
export function parseProcessPid(value: unknown): number | null {
  const parsed = ProcessPidSchema.safeParse(value);
  return parsed.success ? parsed.data : null;
}

export function parseProcessTerminalSize(value: unknown): ProcessTerminalSize | null {
  const parsed = ProcessTerminalSizeSchema.safeParse(value);
  return parsed.success ? parsed.data : null;
}

export function parseProcessSignalName(value: string): ProcessSignalName | null {
...
export function parseProcessLogClientFrame(raw: string): ProcessLogClientFrame | null {
  let value: unknown;
  try {
