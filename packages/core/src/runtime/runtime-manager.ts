...
type InstalledBins = SeededRuntime & { bins: string[] };

/** One queued operation on a canonical target; `done` is also the cleanup key. */
type OpEntry =
  | { kind: 'install'; done: Promise<InstalledBins> }
  | { kind: 'remove'; done: Promise<void> };

export class RuntimeManager {
  private readonly vfs: CredentialedVfs;
  private readonly registry: MinShellRegistry;
  private readonly getHome: () => string;
  private readonly source: RuntimeSource;
  private readonly runnerFactories = new Map<string, RunnerFactory>();
  /**
   ...
   */
  private readonly inflight = new Map<string, OpEntry>();
  /**
   ...
   */
  private readonly resolving = new Set<Promise<RuntimePackage>>();
  ...
  rehydrate(): Promise<{ count: number; bins: string[] }> {
    return rehydrateInstalledRuntimesView(
      this.vfs,
      this.registry,
      this.getHome(),
      (key) => this.runnerFactories.get(key),
    );
  }
   ...
   */
  private async resolveRunnable(spec: string): Promise<RuntimePackage> {
    ...
    if (missing.length === 0) return runtimePackage;
    ...
    if (splitRuntimeSpec(spec).versionOverride === null) {
      ...
      for (const { version: candidate } of [...(offered?.versions ?? [])].reverse()) {
        ...
        if (alternative !== null && this.missingRunners(alternative.manifest).length === 0) return alternative;
      }
    }
    throw new Error(
      `${name}@${version}: runner${missing.length === 1 ? '' : 's'} `
  ...
  private installPrepared(
    resolve: () => Promise<RuntimePackage>,
    options: { force?: boolean; onProgress?: (line: string) => void } | undefined,
  ): Promise<InstalledBins> {
    ...
    const start = async (): Promise<InstalledBins> => {
      ...
      const attempt: Promise<InstalledBins> = after.then(
        () => this.write(runtimePackage, home, options),
        () => this.write(runtimePackage, home, options),
      );
      const entry: OpEntry = { kind: 'install', done: attempt };
  ...
  private async write(
    ...
    options?.onProgress?.(
      `[${name}] manifest: ${manifest.files.length} files, ${(totalBytes / 1024 / 1024).toFixed(1)} MiB`,
    );
  ...
  async uninstall(spec: string): Promise<void> {
    ...
    try {
      ...
      if (this.inflight.get(nameKey) === entry) this.inflight.delete(nameKey);
    }
  }
  ...
  private async remove(name: string, versionOverride: string | null, home: string): Promise<void> {
    ...
    if (versionOverride !== null && matches.length > 0) {
      await rehydrateInstalledRuntimesView(
        this.vfs,
        this.registry,
        home,
        (key) => this.runnerFactories.get(key),
