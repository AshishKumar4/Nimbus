...
function encodeChunkedFrame(bytes: Uint8Array): Uint8Array {
  return concatBytes([
...
type HttpParseOutcome<TMessage> =
 ...
 */
abstract class HttpMessageParser<TMessage> {
  private phase: 'headers' | 'body' | 'done' = 'headers';
  private settled: HttpParseOutcome<TMessage> | null = null;

  private headerBuffer: Uint8Array = EMPTY_BYTES;
  private headerScanOffset = 0;

  protected headerPairs: [string, string][] = [];
  ...
  get outcome(): HttpParseOutcome<TMessage> | null {
    return this.settled;
  }

  /** What this parser is reading, for diagnostics the guest ultimately sees. */
  protected abstract get messageLabel(): string;

  /** Parse the start line; return false to reject the message as malformed. */
  protected abstract parseStartLine(line: string): boolean;

  /** Whether this message may carry a body at all (HEAD, 204, 304, bodyless methods). */
  protected abstract bodyAllowed(): boolean;

  /** Absent framing means "no body" for requests but "read until close" for responses. */
  protected abstract defaultFramingWhenUnframed(): HttpBodyFraming;

  protected abstract buildMessage(body: Uint8Array | null): TMessage;
  ...
  private parseHeaderBlock(headerBytes: Uint8Array): boolean {
  ...
  protected headerValue(name: string): string | null {
 ...
 */
class HttpResponseParser extends HttpMessageParser<ParsedHttpResponse> {
  private status = 200;
  private statusText = '';
  ...
  protected get messageLabel(): string {
    return 'response';
  }

  protected parseStartLine(line: string): boolean {
  ...
  protected bodyAllowed(): boolean {
    return responseCanHaveBody(this.requestMethod, this.status);
  }

  protected defaultFramingWhenUnframed(): HttpBodyFraming {
    return 'until-close';
  }

  protected buildMessage(body: Uint8Array | null): ParsedHttpResponse {
 ...
 */
class HttpRequestParser extends HttpMessageParser<ParsedHttpRequest> {
  private method = 'GET';
  private target = '/';
  ...
  protected buildMessage(body: Uint8Array | null): ParsedHttpRequest {
 ...
 */
class LoopbackClientConnection implements VirtualSocketConnection {
  private readonly parser = new HttpRequestParser();
  /** Response bytes the guest client reads. */
  private readonly inbound = new ByteChunkQueue(Number.MAX_SAFE_INTEGER, 'loopback response buffer');
  private readable: Deferred<void> | null = null;
  private drained: Deferred<void> | null = null;
  ...
  constructor(
    ...
    private readonly route: (port: number, request: Request) => Promise<Response>,
  ...
  write(bytesLike: VirtualSocketBytesLike): number {
    ...
    if (this.dispatched) {
      throw new Error(
        `Nimbus loopback socket: port ${this.port} answered with "Connection: close"; ` +
  ...
  private async dispatch(parsed: ParsedHttpRequest): Promise<void> {
    let request: Request;
    try {
  ...
  private buildRequest(parsed: ParsedHttpRequest): Request {
    const headers = new Headers();
  ...
  private async streamResponse(method: string, response: Response): Promise<void> {
  ...
  private failWith(status: number, detail: string): void {
...
function socketStreamFor(conn: VirtualSocketConnection): VirtualSocketStream {
  return {
...
export class VirtualSocketKernel {
  ...
  private readonly connections = new Map<number, VirtualSocketConnection>();
  private readonly limits: VirtualSocketKernelLimits;
   ...
   */
  async recvAsync(id: number, maxBytes: number): Promise<number[]> {
    const conn = this.connections.get(Number(id));
    if (!conn) return [];
