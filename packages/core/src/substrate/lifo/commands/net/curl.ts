    ...
    try {
      ...
      try {
        ...
        if (kernel) {
          ...
          return await executeKernelRequest(
            kernel, ctx, options, url, headers, requestSignal.signal,
          );
        }
  ...
  for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
    ...
    if (requestUrl !== null) {
      ...
      if (port !== null) {
        ...
        const gateway = await dispatchWorkspaceRequest(
          kernel,
    ...
    if (!follow) {
      return await handleCurlResponse(
        ctx,
        options,
        await drainCurlResponse(options, response, response.url || current),
      );
    }
    ...
    try {
      ...
      return await handleCurlResponse(
        ctx,
        options,
        await drainCurlResponse(options, response, current),
      );
    }
    // Follow decision made: this hop's body is no longer needed.
    cancelStreamBody(response.body);
