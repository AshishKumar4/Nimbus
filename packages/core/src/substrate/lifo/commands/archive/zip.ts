...
const command: Command = async (ctx) => {
  ...
  if (flags.help || ctx.args.length === 0) {
    await ctx.stdout.write('Usage: zip [-rqj] archive.zip file1 [file2 ...]\n');
    await ctx.stdout.write('  -r   recurse into directories\n');
    await ctx.stdout.write('  -q   quiet\n');
    await ctx.stdout.write('  -j   junk paths, store each entry by basename\n');
  ...
  if (rejected.length > 0) {
    await ctx.stderr.write(`zip: invalid option -- '${rejected[0].replace(/^-+/, '')}'\n`);
    return 1;
  }
  ...
  if (!archiveFile || files.length === 0) {
    await ctx.stderr.write('zip: no files to archive\n');
    return 1;
  }
  ...
  try {
    ...
    if (!flags.quiet) {
      for (const entry of zipEntries) {
        await ctx.stdout.write(`  adding: ${entry.path}${entry.isDirectory ? '/' : ''}\n`);
      }
    }
  } catch (e) {
