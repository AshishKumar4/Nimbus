...
async function lifoInit(ctx: CommandContext): Promise<number> { const name = ctx.args[1];
if (!name) {
...
(await ctx.vfs.writeFile(join(pkgDir, 'package.json'),
...
async function lifoLink(ctx: CommandContext, registry: CommandRegistry): Promise<number> { const pathArg = ctx.args[1] || '.';
...
async function lifoUnlink(ctx: CommandContext): Promise<number> { const name = ctx.args[1];
if (!name) {
