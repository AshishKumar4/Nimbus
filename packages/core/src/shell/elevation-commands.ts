...
function symbolicUmask(mask: number): string {
  ...
  const classText = (shift: number): string => {
...
export function createUmaskCommand(): Command {
  return async (ctx) => {
    if (!ctx.cred) {
...
async function targetCredential(ctx: CommandContext, name: string) {
  if (!ctx.cred) throw new Error('process credential is unavailable');
  const user = (await findUnixUser(ctx.vfs, name));
  if (!user) throw new Error(`unknown user: ${name}`);
  if (user.uid === 0) return CRED_KERNEL;
...
export function createSudoCommand(): Command {
  return async (ctx) => {
    let userName = 'root';
    let index = 0;
    if (ctx.args[index] === '-u') {
...
export function createSuCommand(): Command {
  return async (ctx) => {
    let index = 0;
    while (ctx.args[index] === '-' || ctx.args[index] === '-l' || ctx.args[index] === '--login') index++;
