...
import { RPC_FRAME_TYPE } from './protocol.js';
function sendIfOpen(connection, frame) {
    try {
        connection.send(JSON.stringify(frame));
        return true;
    }
    catch (e) {
        if (e instanceof TypeError && e.message.includes('WebSocket send() after close'))
            return false;
        throw e;
    }
}
...
export async function dispatchRpc(target, connection, frame) {
    const method = target[frame.method];
    try {
        if (typeof method !== 'function')
            throw new Error(`Method ${frame.method} does not exist`);
        if (!isCallable(method))
            throw new Error(`Method ${frame.method} is not callable`);
        if (callableMetadata(method)?.streaming) {
            ...
            try {
                ...
                if (!stream.isClosed)
                    stream.error(errorMessage(e));
            }
            return;
        }
        const result = await method.apply(target, frame.args);
