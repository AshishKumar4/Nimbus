...
export const SCHEDULE_RETRY_DEFAULTS = {
    maxAttempts: 3,
    baseDelayMs: 100,
    maxDelayMs: 3000,
};
...
export class ScheduleStore {
     ...
     * alarm).
     */
    create(when, callback, payload, options = {}) {
    ...
    every(intervalSeconds, callback, payload, options = {}) {
        if (typeof callback !== 'string' || callback === '') {
            throw new Error('loom: a schedule callback must be a method name');
        }
        ...
        const now = options.now ?? Date.now();
        return this.insert(callback, payload, { type: 'interval', time: now + intervalSeconds * 1000, delay_s: null, cron: null, interval_s: intervalSeconds }, options.retry, now);
    }
    insert(callback, payload, partial, retry, now) {
        this.ensureSchema();
        const id = crypto.randomUUID();
        const payloadJson = payload === undefined ? null : JSON.stringify(payload);
        this.ctx.storage.sql.exec(`INSERT INTO loom_schedules (id, callback, payload, type, time, delay_s, cron, interval_s, attempt, retry, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`, id, callback, payloadJson, partial.type, partial.time, partial.delay_s, partial.cron, partial.interval_s, retry === undefined ? null : JSON.stringify(retry), now);
        return toSchedule({ id, callback, payload: payloadJson, attempt: 0, retry: retry === undefined ? null : JSON.stringify(retry), ...partial });
    }
    byId(id) {
        this.ensureSchema();
        const rows = [...this.ctx.storage.sql.exec(`SELECT * FROM loom_schedules WHERE id = ?`, id)];
        return rows.length > 0 ? toSchedule(RowSchema.parse(rows[0])) : undefined;
    }
    list(criteria = {}) {
        this.ensureSchema();
        const where = [];
        ...
        if (criteria.timeRange?.start !== undefined) {
            where.push('time >= ?');
            bindings.push(criteria.timeRange.start.getTime());
        }
        if (criteria.timeRange?.end !== undefined) {
            where.push('time <= ?');
            bindings.push(criteria.timeRange.end.getTime());
        }
        const rows = [...this.ctx.storage.sql.exec(`SELECT * FROM loom_schedules${where.length > 0 ? ` WHERE ${where.join(' AND ')}` : ''} ORDER BY time, id`, ...bindings)];
        return rows.map((row) => toSchedule(RowSchema.parse(row)));
    }
    ...
    nextDue() {
        this.ensureSchema();
        const rows = [...this.ctx.storage.sql.exec(`SELECT MIN(time) AS next FROM loom_schedules`)];
     ...
     */
    async dispatchDue(target, now, alarmInfo, onError) {
        this.ensureSchema();
        const due = [...this.ctx.storage.sql.exec(`SELECT * FROM loom_schedules WHERE time <= ? ORDER BY time, id`, now)].map((row) => RowSchema.parse(row));
        for (const row of due) {
            const schedule = toSchedule(row);
            const attempt = row.attempt + 1;
            const method = target[row.callback];
            const callable = typeof method === 'function';
            try {
                if (!callable)
                    throw new Error(`loom: schedule callback 'this.${row.callback}' is not a method on this actor`);
                const payload = row.payload === null ? undefined : JSON.parse(row.payload);
                const invocation = { schedule, attempt, alarmInfo };
                await method.call(target, payload, invocation);
                ...
                if (callable && attempt < policy.maxAttempts) {
                    ...
                    this.ctx.storage.sql.exec(`UPDATE loom_schedules SET attempt = ?, time = ? WHERE id = ?`, attempt, now + delay, row.id);
                }
                else {
...
function toSchedule(row) {
    const payload = (row.payload === null ? undefined : JSON.parse(row.payload));
    const retry = row.retry === null ? undefined : JSON.parse(row.retry);
